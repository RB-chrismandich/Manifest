#!/usr/bin/env bash
# spec_review.sh — cross-reference spec/plan/tasks artifacts with one reviewer.
# Analysis-only: never edits artifacts. Front-end-agnostic — the /spec-review
...
SPEC_REVIEW_CONFIG="${SPEC_REVIEW_CONFIG:-${HOME:-}/.claude/config/model_policy.yml}"
...
# resolve_review_model -> model name on stdout, or empty.
...
# run_reviewer PROMPT -> raw reviewer output. The prompt body is stdin and the
# short instruction requests the contract expected by format_findings.
run_reviewer() {
    ...
    printf '%s' "$prompt" | "$SPEC_REVIEW_CLI" ${cli_args[@]+"${cli_args[@]}"}
}
...
review() {
    ...
    echo "[spec-review] Cross-referencing project artifacts with one reviewer…"
    local prompt raw
