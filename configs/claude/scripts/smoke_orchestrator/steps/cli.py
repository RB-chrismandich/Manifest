...
def run(step: dict, *, timeout_s: float) -> StepOutcome:
    ...
    try:
        proc = subprocess.run(  # noqa: S603 - arg-array, shell=False by construction
    ...
    except OSError as exc:  # e.g. PermissionError / IsADirectoryError — fail the step, not the run
        return StepOutcome(False, f"could not execute {args[0]!r}: {type(exc).__name__}")
    expect = step.get("expect_exit", 0)
    if proc.returncode != expect:
        stderr = (proc.stderr or "").strip()[:200]
        return StepOutcome(False, f"exit {proc.returncode} (expected {expect}); stderr: {stderr}")
...
def _extract(captures: dict, stdout: str) -> dict:
    ...
    for name, pattern in captures.items():
        ...
        if m is None:
            raise CaptureError(f"cli capture {name!r}: pattern {pattern!r} matched nothing in stdout")
