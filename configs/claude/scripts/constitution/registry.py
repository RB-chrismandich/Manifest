"""Load and validate code_constitution.yml into declared records.
...
try:  # pyyaml ships with the deployed runtime but is not guaranteed elsewhere.
    import yaml
except ImportError:  # pragma: no cover - exercised by the missing-dep path only
    yaml = None  # type: ignore[assignment]

# scripts/constitution/registry.py -> parents[2] is the config root's parent,
# which resolves identically in-repo (configs/claude/) and deployed (~/.claude/).
DEFAULT_PATH = Path(__file__).resolve().parents[2] / "config" / "code_constitution.yml"
ENV_OVERRIDE = "CODE_CONSTITUTION_YML"
...
def load(path: Path | None = None) -> Registry:
    ...
    if yaml is None:
        raise RegistryError(
            f"pyyaml is required to read {target}; install it or set {ENV_OVERRIDE}"
        )
    try:
        ...
        raise RegistryError(f"registry {target} is not valid YAML: {err}") from err
