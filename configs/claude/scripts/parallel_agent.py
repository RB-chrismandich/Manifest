...
async def main():
    ...
    epilog_text = """
Examples:
  # Basic prompt using all available agents
  python3 parallel_agent.py "Explain quantum computing"

  # Review a specific file with advanced models
  python3 parallel_agent.py --review src/main.py --claude-model opus --gemini-model pro

  # Analyze a file using only Claude and Cursor
  python3 parallel_agent.py --analyze tests/security.py --claude-only --cursor-only

  # Run without streaming, saving JSON output to a custom directory
  python3 parallel_agent.py "Generate unit tests" --json --no-stream --output ./results
"""
    ...
    model_group.add_argument("--claude-model", default="sonnet", help="Claude model tier (default: sonnet)")
    model_group.add_argument("--gemini-model", default="flash", help="Gemini model tier (default: flash)")
    model_group.add_argument("--cursor-model", default="flash", help="Cursor model tier (default: flash)")
    model_group.add_argument("--codex-model", default="auto", help="Codex model tier (default: auto)")

    agent_group = parser.add_argument_group("Agent Toggles")
    ...
    adv_group.add_argument("--validate", action="store_true", help="Validate results")
    adv_group.add_argument("--synthesize", action="store_true", default=True, help="Enable synthesis for low consensus")
    adv_group.add_argument("--timeout", type=int, default=None, help="Timeout per agent in seconds (Defaults: review=600, analyze=900, improve=300, prompt=600)")
