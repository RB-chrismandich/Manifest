...
Examples:
  ...
  # Code review with specific models
  python parallel_agent.py --review src/main.py --claude-model opus --cursor-model advanced

  # Run only specific agents
  python parallel_agent.py "Test prompt" --claude-only --gemini-only

  # Output complete results to a directory
  python parallel_agent.py "Generate docs" --output ./docs_output --full-output

  # Pre-flight check
  python parallel_agent.py --check-credits
"""
    )
    ...
    behavior_group.add_argument("--validate", action="store_true", help="Validate results")
    behavior_group.add_argument("--synthesize", action="store_true", default=True, help="Enable synthesis for low consensus")
    behavior_group.add_argument("--timeout", type=int, default=None, help="Timeout per agent (seconds). Defaults: review=600, analyze=900, improve=300, prompt=600")
    ...
    agent_group = parser.add_argument_group("Agent Control")
