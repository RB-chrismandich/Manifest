...
Examples:
  # Basic prompt
  python parallel_agent.py "Refactor the login component to use hooks"

  # Code review mode
  python parallel_agent.py --review src/auth.py

  # Run only specific agents
  python parallel_agent.py "Analyze performance" --claude-only --cursor-only

  # Use specific models with JSON output
  python parallel_agent.py "Generate a test plan" --claude-model opus --json
'''
    )
    ...
    exec_group.add_argument("--synthesize", action="store_true", default=True, help="Enable synthesis for low consensus")
    exec_group.add_argument("--timeout", type=int, default=None, help="Timeout per agent (seconds). Defaults: review=600, analyze=900, improve=300, prompt=600")
