...
class Orchestrator:
    ...
    def _calculate_consensus(self, results: Dict) -> Dict:
        ...
        # ⚡ Bolt: Use collections.Counter and itertools.chain.from_iterable for ~20% faster counting
        output_word_sets = [
        ...
        if not output_word_sets:
            ...
            all_words_len = len(set.union(*output_word_sets))
            if all_words_len == 0:
                consensus_score = 0
            else:
                word_counts = collections.Counter(itertools.chain.from_iterable(output_word_sets))
