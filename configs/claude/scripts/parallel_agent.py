...
class Orchestrator:
    ...
    async def _write_output_files(
        ...
        def write_agent(agent_name, agent_result):
        ...
        # Execute file writes concurrently to prevent event loop blocking
        tasks = [
            asyncio.to_thread(write_agent, name, res)
            for name, res in result["agents"].items()
        ]
        tasks.extend([asyncio.to_thread(write_json), asyncio.to_thread(write_md)])
