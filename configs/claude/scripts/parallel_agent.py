...
async def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Run a basic prompt across all available agents
  python parallel_agent.py "Explain quantum computing"

  # Review a file with specific models
  python parallel_agent.py --review src/main.py --claude-model opus --cursor-model advanced

  # Run only Claude and output as JSON
  python parallel_agent.py "Generate a React component" --claude-only --json

  # Analyze a file for bugs/security issues without streaming
  python parallel_agent.py --analyze src/auth.py --no-stream
"""
    )
    ...
    orch_group = parser.add_argument_group("Orchestration Options")
    orch_group.add_argument("--validate", action="store_true", help="Validate results")
    orch_group.add_argument("--synthesize", action="store_true", default=True, help="Enable synthesis for low consensus")
    orch_group.add_argument("--timeout", type=int, default=None, help="Timeout per agent (seconds). Defaults: review=600, analyze=900, improve=300, prompt=600")
