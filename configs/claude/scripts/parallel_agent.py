...
class Orchestrator:
    ...
    def _calculate_consensus(self, results: Dict) -> Dict:
        ...
        word_counts = Counter(itertools.chain.from_iterable(word_sets))

        # Calculate consensus as % of words appearing in multiple outputs
        all_words_len = len(word_counts)
        if not all_words_len:
            consensus_score = 0
        else:
            # We can calculate common words simply by all words minus unique words (count == 1)
            common_words = all_words_len - sum(1 for count in word_counts.values() if count == 1)
            consensus_score = int((common_words / all_words_len) * 100)
