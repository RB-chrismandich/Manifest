...
class Orchestrator:
    ...
    async def _write_output_files(
        ...
        """Write output files to disk with sandbox-aware fallback using threads to prevent event loop blocking"""
        output_dir = self._resolve_output_dir(custom_output_dir)

        # Offload directory creation to thread
        await asyncio.to_thread(output_dir.mkdir, parents=True, exist_ok=True, mode=0o700)

        output_files = {}

        # Offload file writes to thread
        def write_all_files():
        ...
        # Execute blocking file operations in thread pool
        output_files = await asyncio.to_thread(write_all_files)
