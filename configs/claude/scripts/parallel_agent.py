...
class Orchestrator:
    ...
    async def _write_output_files(
        ...
        def write_agent_output(agent_name, agent_result, filepath):
        ...
        for agent_name, agent_result in result["agents"].items():
            ...
            write_tasks.append(
                asyncio.to_thread(
                    write_agent_output, agent_name, agent_result, output_file
                )
            )
            output_files[agent_name] = str(output_file)

        def write_json_output(filepath):
            with open(filepath, "w") as f:
                json.dump(result, f, indent=2)

        # Write JSON results
        json_file = output_dir / f"results_{timestamp}.json"
        write_tasks.append(asyncio.to_thread(write_json_output, json_file))
        output_files["json"] = str(json_file)

        def write_md_summary(filepath):
            with open(filepath, "w") as f:
        ...
        write_tasks.append(asyncio.to_thread(write_md_summary, md_file))
        output_files["summary"] = str(md_file)

        # Run all file writing tasks concurrently
