...
Machine copy: `~/.claude/config/code_constitution.yml` (`languages.go`).
...
it needs in the same change, not after the first red run. `templates/scaffold/go/` is the starting point, but its
