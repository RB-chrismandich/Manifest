...
the user) invokes `/session-checkpoint` manually when the thresholds below are reached.

## Thresholds

- **80–95% context used**: run `/session-checkpoint` to compress the session into a
  scratchpad summary + memory updates before quality degrades.
- **Above 95%**: stop new work, run `/session-checkpoint`, and recommend starting a
...
Run /session-checkpoint to preserve session history before continuing.
```
