...
**Purpose**: Provide AI agents with structure, deployment, and testing context
...
- **OpenTofu MCP**: OpenTofu/Terraform registry lookups, provider and module
  documentation, resource/datasource reference for Infrastructure as Code.
...
│   │   └── mcp_servers.yml          # MCP server registry (OAuth-capable)
│   ├── .plans/                      # Plan management (template, archive)
│   ├── settings.local.json          # Default permissions and MCP config
│   └── scripts/parallel_agent.sh    # Parallel agent orchestration script
...
│   ├── config.sh                    # Argument parsing helpers
...
| `configs/cursor/rules/orchestration.mdc` | Main orchestration guide |
| `configs/gemini/GEMINI.md` | Main orchestration guide for Gemini |
| `configs/codex/AGENTS.md` | Main orchestration guide for Codex |
| `configs/claude/scripts/parallel_agent.sh` | Parallel orchestration script |
| `configs/claude/config/command_config.yml` | Tool policies and models |
...
skills). Skills are invoked as slash commands (e.g., `/refactor-python src/`).
...
| `/plan-manage` | Plan lifecycle with parallel agents | CONDITIONAL |
| `/project-commit` | Full commit pipeline | CONDITIONAL |
| `/refactor-go` | Go codebase analysis | ALWAYS |
| `/refactor-node` | Node.js/TypeScript analysis | ALWAYS |
| `/refactor-python` | Python codebase analysis | ALWAYS |
| `/refactor-shell` | Bash/Shell script analysis | ALWAYS |
| `/refactor-terraform` | Terraform IaC analysis | ALWAYS |
| `/scaffold` | Initialize new project | NO |
...
`generate_cursor_rules.sh`. Each skill produces a corresponding `.mdc` rule in
...
`~/.gemini/skills`). To add a new skill, follow the Claude Code skill
instructions above.
