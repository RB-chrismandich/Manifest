...
```text
configs/                             # Deployment source configs (to ~/)
...
│   └── scripts/parallel_agent.sh    # Main parallel agent script
...
.claude/                             # Repo-specific config (no override)
├── CLAUDE.md                        # Developer guide for working here
...
├── lib/                             # Shared logic split by concern
│   ├── common.sh                    # Output/prompts/symlink helpers
│   ├── modules.sh                   # Hook registry + module loader
│   ├── platform.sh                  # Platform detection + timeout helpers
│   ├── config.sh                    # Arg parsing + config helpers
│   ├── install.sh                   # CLI install routines
│   ├── auth.sh                      # Authentication + state setup
│   ├── deploy.sh                    # Deploy/verify/summary routines
│   └── mcp.sh                       # MCP configuration/install routines
└── modules/README.md                # Custom bootstrap extensions
...
| `configs/gemini/GEMINI.md` | Gemini orchestration guide |
| `configs/codex/AGENTS.md` | Codex orchestration guide |
| `configs/claude/scripts/parallel_agent.sh` | Main orchestration script |
...
~/.claude/scripts/parallel_agent.sh --json --timeout 600 --review /path/to/file
...
Lifecycle: `CREATE -> ACTIVE -> COMPLETED (.archive/) | ABANDONED (.abandoned/)`
...
3. Rule auto-attaches when files matching `globs` are referenced (after
   deploying)
