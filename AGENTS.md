...
- **Uncategorized**: `/automation-rework-breakeven` · `/cli-help-before-dependency-checks` · `/deploy-reconcile` · `/errexit-safe-shell-counters` · `/false-green-check-audit` · `/lifecycle-run` · `/merge-stacked-pr-chain` · `/pin-known-bug-test-survives-fix` · `/pr-smoke` · `/reproduce-gated-ci-failure-locally` · `/shell-sete-silent-abort-audit` · `/spec-implement-loop`
