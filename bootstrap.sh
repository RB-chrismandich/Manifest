    ...
    if prompt_yes_no "Set up Gemini API key instead?"; then
        ...
        if [[ -n "$api_key" ]]; then
            ...
            if [[ -n "$shell_profile" ]]; then
                # Security: Write to a separate file with 600 permissions instead of shell profile
...
# Gemini API Key
# Generated by bootstrap.sh on $(date)
export GEMINI_API_KEY='$safe_api_key'
EOF

                # Add source command to shell profile if not present
                if ! grep -q "gemini_env.sh" "$shell_profile"; then
                    {
                        echo ""
                        echo "# Gemini API Key (securely stored in $env_file)"
                        echo "if [[ -f \"$env_file\" ]]; then source \"$env_file\"; fi"
                    } >> "$shell_profile"
                fi

                export GEMINI_API_KEY="$api_key"
                print_success "API key saved to $env_file (permissions 600)"
                print_success "Added source command to $shell_profile"
