...
- **Reuse-vs-new + final name** → NEW skill `deploy-reconcile`, modeled on `branch-clean`; do not extend sync-configs / health-check / deploy-drift-root-cause / the retired cross-harness coordinator (Topic 7).
...
**Rationale.** The protection boundary already exists implicitly: `restore_runtime_state` excludes the repo-owned top-level entries and restores everything else as "user/runtime state" — exactly the orphan-vs-protected line. A dedicated `reconcile.yml` matches the `skillclaw.yml`/`linear_triage.yml`/`model_policy.yml` scope-isolation precedent, auto-deploys via the existing rsync, and is itself a project source so it can never be flagged as its own orphan. Additive-only union fails toward KEEP. The `~/.manifest` override lets users add protections without editing the redeploy-overwritten tree (honors P-I).
...
**Rationale.** P-IV is satisfied by a skill wrapping a dedicated script (branch-clean precedent), with no logic added to the retired cross-harness coordinator. The dual-mode + `--yes`/`--json`/`err()`/≤15-line-help shape is proven in `branch_clean.sh`/`label_sync.sh`. P-V fail-open is met by the `|| …` guard under `set -e`. Report-only (FR-006) is structural: no `--remove` + exit 0 on orphans. Backup under `~/.manifest` reuses the out-of-scope state root.
...
**Decision.** Ship as a NEW skill `deploy-reconcile` (`/deploy-reconcile`) wrapping new `configs/claude/scripts/deploy_reconcile.sh`, modeled on `branch-clean` (preview default, `--remove`+confirm+`--yes`, config-driven protection, Tier 1 on the destructive path). Do NOT extend sync-configs, health-check, deploy-drift-root-cause, `verify_installation`, or the retired cross-harness coordinator. Registry/doc work: (1) `.retired skill supply/skills/deploy-reconcile/SKILL.md` name+description frontmatter; (2) a `deploy-reconcile:` `tool_policies` block in `command_config.yml` (allowed Read/Glob/Grep/Bash; OMP dispatch conditional; validation_tier 1; no sub-agents during mutation — copy branch-clean shape) — **protection DATA lives in `reconcile.yml`, not a command_config.yml block (Topic 2 / FR-014).**

**Rationale.** Each existing capability addresses a different concern and the destructive opt-in mode is contract-incompatible with the read-only diagnostics: sync-configs and health-check forbid Write/Edit/Task and use no OMP sub-agents at tier 2 — adding `--remove` violates that contract and neither does a deployed-vs-project orphan compare. deploy-drift-root-cause is the INVERSE problem (state MISSING from a deploy) vs this (state EXTRA/orphaned). `verify_installation` only asserts a fixed required-files list is present. The closest prune (`deploy_home_skills` + `.deployed-skills`, gate_graphify_skill) is stateful, skills-only, single-root, with no KEEP/REMOVE/protection/backup/preview. branch-clean is the correct precedent (dry-run default, `--apply`+confirm).
