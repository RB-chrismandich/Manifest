...
- [X] T027 Constitution Principle II gate: dispatch an OMP reviewer wave for cross-verification of the full diff (security-relevant guidance + >200 lines) on the key changed files (absolute paths); attach the review verdict to the PR description; judge completed reviewers only.
