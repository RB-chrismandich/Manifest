...
- [x] T017 [US1] Write `tests/bats/apm_deploy_isolated.bats` — **before** the migration, and demonstrate each test fails against the current pipeline. Every case asserts its precondition first: reproduce-from-package-and-lockfile, then diff against an **expected file manifest** (a non-empty tree assertion, so an install that silently no-ops cannot produce an empty-vs-empty pass); assert old-file **present** → rename → re-deploy → assert old-file **absent** and new-file present (so a rename that never took hold cannot look like successful cleanup); hand-edit → retained **and surfaced in the deploy workflow output**; re-deploy → byte-identical. Include a corrupted-lockfile case that must fail loudly, not deploy partially. **Enumerate the named historical drift instances in the test file** — the `mcpServers` clobber, the `__pycache__` orphan, the unpruned `~/.cursor/rules`, the toggle-off skill copy, the stale `COMMANDS.md` — as comments on the cases that generalize them, so SC-008's "each class has a regression test" is traceable to the specific bugs it claims to have killed rather than asserted at the class level. → FR-004..FR-007, FR-023, SC-001..SC-004, SC-008
  - **Done 2026-07-27, before T018 as required.** Demonstrated SKIPPING (not passing) while no package existed — a file that silently passes zero cases is not evidence. Every case asserts its precondition first; the named historical drift instances are mapped in the header to the cases that generalize them (SC-008).
  - One case changed meaning mid-flight: T017 specified "hand-edit → **retained** and surfaced". Constitution v3.0.0 amended V.4 from preserve-and-report to **detect**-and-report, because the package manager performs the write and preservation is not expressible. The assertion moved with the principle.

### Implementation

- [x] T018 [US1] Author `apm.yml` (pinned tool version, explicit targets) and `.apm/` for **one** domain, per T013's arrangement; publish it; commit `apm.lock.yaml`. → FR-007, FR-017
  - **Done 2026-07-27 — PUBLISHED.** `apm.yml` at the repo root: one domain, `targets: [claude]` only, because cell (i) measured that apm preserves symlinked targets and naming five harnesses would turn one shared tree into five free to drift.
  - Published as tag **`v0.0.0-apm-preview.1`** on `RB-chrismandich/Manifest` — deliberately a prerelease, and the repo's FIRST tag, so it cannot read as a product release. Gate record `subject_sha256:97c1b732…` `result:"pass"` at `23:11:33Z` precedes the push (SC-011 holds).
  - **The repo is PUBLIC** — confirmed after the fact by an install that succeeded with an empty token. That makes the pre-publish scan load-bearing rather than ceremonial, and it is why the five false positives were worth fixing properly instead of bypassing.
- [x] T019 [US1] Run T017 to green, iterating the package. Record actual command output — a plausible-looking config is not evidence. → FR-004..FR-007
  - **Done 2026-07-27 — 8/8 green on real content**, installed BY NAME from the published tag (`apm install --global 'RB-chrismandich/Manifest#v0.0.0-apm-preview.1'` → 108 skills). Actual command output recorded, not a plausible-looking config.
  - Note the asymmetry this proves: a **local-path** install of the checkout FAILS (apm hard-rejects `configs/claude/.venv`'s escaping symlink) while the **git-host** install succeeds, because that symlink is untracked and absent from the published tree.
...
- [x] T020 [US1] Re-run `apm_ownership_boundary.bats` **at this exact point** — the first moment two pipelines are simultaneously live is the highest-risk instant for a double write, and v1 only checked at deletion points. → FR-014
  - **Done 2026-07-27, at the actual moment** — with the published package installed and the registry still empty, i.e. both pipelines genuinely live on `~/.claude/skills`. `apm_ownership_report.sh` returned **DOUBLE-CLAIMED, exit 1**: the highest-risk state, detected rather than assumed away. Boundary suite 13/13 at that same point.
- [x] T021 [US1] Verify drift detection: compare `apm audit` against `deploy_reconcile.sh` on a deliberately-drifted tree and document any class the new tool misses **before** relying on it. → FR-008
  - **Done 2026-07-27 — the side-by-side is the evidence.** On one drifted tree (a hand-edited `SKILL.md` plus an orphan directory): `apm_drift_report.sh` → `MODIFIED .claude/skills/env-check/SKILL.md` with both hashes; `apm audit --file <lockfile>` on the **same tree** → `1 file(s) scanned -- no issues found`.
  - **The class apm does not cover is deployed-file CONTENT drift**, and this is the empirical proof rather than an inference from `drift.py`'s docstring. It is exactly the gap Constitution V.4 now obliges Manifest to close itself, which `apm_drift_report.sh` does.
- [x] T022 [US1] Add `tests/bats/apm_supply_chain.bats` — an **independent** tree hash computed by something `apm` does not control, wired as a standing gate. The lockfile is generated by `apm` and `apm audit` reads it back; alone they are one party checking its own work. → FR-036, E9
  - **Done 2026-07-27** — landed as `tests/bats/apm_independent_hash.bats` (the filename in the task text was already taken by T042's fail-closed suite). Uses `apm_hash_lib.sh`'s NUL-safe walk, which apm has no part in, and **asserts that independence** rather than claiming it: one test greps the lib for any `apm install|audit|list|pack` call, so if it ever starts shelling out to apm the gate silently becomes apm checking its own work again and that test goes red.
  - Discriminates on content change, addition, and removal, and is path-independent (an identical tree elsewhere hashes the same), so a deploy verified in a sandbox stays verified in a real home. The content-change case matters most: content drift inside a deployed file is not one of apm's four drift categories, so this hash is the only thing that would ever notice a hand-edit.
- [x] T023 [US1] Register the deploy suite as this feature's **Verify-gate smoke coverage**: missing coverage is never a pass. → Constitution VI (gate-mapped, not FR-mapped)
  - **Done 2026-07-27** — `apm-ownership-single-owner` added to `smoke-catalog/manifest.yaml` (tier Lite, hermetic temp home, never touches the real `~/.claude`). It asserts **both directions**: exit 0 on the shipped legacy-owned state, and a hard failure if the gate does *not* detect a gated-but-undeployed domain. A coverage entry that could only pass would satisfy the letter of "missing coverage is never a pass" while proving nothing.
  - Appended as text, not via `yaml.safe_dump` — a first attempt round-tripped the file and stripped the header comments, which the catalog's own header warns about. Verified by parsing the entry back out and executing its command.
...
- [x] T024 [US2] Extend `apm.yml` to all five harnesses, **naming `antigravity` explicitly** (excluded from the `all` meta-target, E2), and assert every supported harness appears in the resolved target list. → FR-011
  - **CLOSED VOID 2026-07-27.** Superseded by T013's measured decision: cell (i) recorded that apm preserves symlinked targets, so the four sibling homes already inherit a `targets: [claude]` deploy. Implementing T024 literally would turn one shared tree into five free to drift — the outcome FR-033 exists to prevent. T024 predates the measurement.
  - ⚠️ **BLOCKED ON A SPEC CONFLICT — do not implement as written.** T024 says to extend `apm.yml` to all five harnesses. T013 decided the opposite, *on measured evidence*: cell (i) recorded that apm **preserves** a symlinked target directory, so `~/.cursor/skills` → `~/.claude/skills` and its siblings already inherit a `targets: [claude]` deploy. Naming five harnesses replaces one shared tree with five independent copies free to drift apart — the outcome **FR-033 exists to prevent**, and the disk/build cost T013 recorded as N=1 becomes N=5.
  - T024 predates the measurement. Resolve explicitly before implementing: either amend T024 to "assert the fan-out covers every supported harness" (consistent with T013/FR-033), or overturn T013 with a stated reason. Silently doing what T024 literally says would undo a decision that was measured rather than assumed.
- [x] T025 [US2] Build the **equivalence harness** for the three per-harness generators. "Two independent clean runs" means **fresh scratch dir, fresh clone, no shared build cache** — name the nondeterminism being controlled for, or a second run from the same cache proves nothing. Enumerate intentional differences in writing. → FR-010
  - **CLOSED VOID 2026-07-27 — no subject exists.** apm emits **0** `.mdc` files for a 108-skill package at any target setting (`cursor (instructions)` is unsupported); the legacy generator emits 110. An equivalence harness would compare 110 against 0. See `us2-blocked.md`.
  - ⛔ **BLOCKED 2026-07-27 — measured: there is nothing to be equivalent to.** A real 108-skill package installed with `--target claude,cursor` wrote **0 files** to `~/.cursor`; apm names `cursor (instructions)` as unsupported, and `.mdc` rules ARE Cursor's instructions primitive. `generate_cursor_rules.sh` produces 110 `.mdc` files today; apm produces none, at any target setting. See `us2-blocked.md`.
- [x] T026 [US2] **Functional consumption check**: deploy the new build's output into an isolated `HOME` and confirm a real harness actually loads it. Content-equal is not behavior-equal, and a static diff alone should not license deleting the only working pipeline. → FR-010
  - **CLOSED VOID 2026-07-27.** Nothing to load: there is no APM-produced cursor output to deploy into a home. Same measurement as T025.
  - ⛔ **BLOCKED 2026-07-27 — measured: there is nothing to be equivalent to.** A real 108-skill package installed with `--target claude,cursor` wrote **0 files** to `~/.cursor`; apm names `cursor (instructions)` as unsupported, and `.mdc` rules ARE Cursor's instructions primitive. `generate_cursor_rules.sh` produces 110 `.mdc` files today; apm produces none, at any target setting. See `us2-blocked.md`.
- [x] T027 [US2] Run the **full CI mirror now**, before any deletion — `shellcheck`, `yamllint`, `bats tests/bats/`, `pytest tests/python/`, and the changed-file gate with `--from-ref origin/main`. Review moved this ahead of the irreversible step; a repo-wide change drags never-gated files into the changed-file gate, and discovering that after deletion is the wrong order. → SC-005
  - **Done 2026-07-27 — all five legs green, before any deletion.** `shellcheck` (bootstrap + all shipped scripts) clean; `yamllint` clean; `bats` **1397/1397**; `pytest` **745 passed**; and the leg that actually matters here, the changed-file gate `pre-commit run --from-ref origin/main --to-ref HEAD`, **exit 0**. That last one is why the task was moved ahead of the irreversible step: a repo-wide change drags never-gated files into the gate, and finding that out after deleting 109 files is not recoverable.
...
- [x] T030 [US2] Remove the **109** committed `.mdc` artifacts from version control and update hygiene gates that assume their presence — including the end-of-file-fixer/double-newline interaction and any count-based doc assertions. → FR-012
  ...
  - ⛔ **MUST NOT RUN — no replacement exists (measured 2026-07-27).** This would delete a live capability: apm emits no `.mdc` output, so nothing takes over. T025/T026 were made preconditions of this task precisely to catch that, and they did — before anything was deleted. Close as VOID or amend US2; see `us2-blocked.md`.
- [x] T031 [US2] Confirm one source edit propagates to all five harness outputs in a single build, and that `bats tests/bats/` + `pytest tests/python/` pass with the four generators gone. → FR-009
  - **CLOSED VOID 2026-07-27.** Depends on the five-harness build T024/T025 would have produced. The property that *does* hold is narrower and is already proven: one source edit propagates to `~/.claude/skills` and reaches the other four homes by symlink (T005 cell (i), `apm_dev_sync` verified on 108 real skills).

---

## Phase 5: User Story 3 - `bootstrap.sh` narrows (P3)

- [x] T032 [US3] Remove configuration-content deployment for migrated domains from `bootstrap/lib/deploy.sh`, leaving CLI installation, auth, MCP config, toggles, and the settings merge untouched. Re-run the ownership test. → FR-013, SC-006
  - **CLOSED NOT-APPLICABLE 2026-07-27.** Removes deployment for *migrated* domains. US2 migrated none, and `skills` is deliberately un-activated (`apm_domains.yml` is `domains: []`). Stripping `deploy_configs()` now would remove deployment for domains nothing else deploys. Reopen if and when SC-006 is decided and a domain is genuinely handed over.
- [x] T033 [US3] Verify service-toggle behavior: a disabled component must be **absent** from the home tree after deploy. Assert absence, not merely that a copy step wasn't invoked. → FR-016
  - **Done 2026-07-27** — three tests in `apm_independent_hash.bats`. Asserts **absence on disk** (`find … | wc -l` = 0) for both the ownership gate and the deploy selector, not that a copy step went unexecuted: "the function returned early" is a statement about our code, "the file is not there" is a statement about the user's machine. A third test proves the absence assertions are not vacuous by deploying through the same call ungated and asserting 2 files land.
- [x] T034 [US3] Prove the rollback using T011's selective deploy **and T053's reclamation step**: take a partially migrated machine, follow the procedure, confirm a working configuration with no legacy-owned files stripped **and no APM-written orphans left behind** — diff the rolled-back tree against a never-migrated one and account for every difference. A rollback that returns the writer but not the file ownership has not rolled back. → FR-019, FR-014
  - **Done 2026-07-27** — `tests/bats/apm_rollback_e2e.bats`, 7/7 on a genuinely half-migrated fixture (gated + apm-deployed, including one primitive the legacy pipeline would never write). Acceptance is a **diff against a never-migrated control**, not a spot-check: spot-checking "are my skills back?" passes while an APM-written orphan sits in the tree owned by nobody — the exact untracked-hybrid state this feature exists to eliminate. A companion test plants a difference and asserts the diff catches it, because a diff of two empty trees also passes.
- [x] T035 [US3] Verify the constitution amendment (v2.0.0) is consistent with the shipped state, and update the guidance that still names `./bootstrap.sh --reconfigure` as the drift-correction command (`CLAUDE.md`, `configs/claude/CLAUDE.md`, `docs/CONFIGURATION.md`, `docs/GETTING_STARTED.md`). → Constitution I/V
  - **Done 2026-07-27 — result is INCONSISTENT, and it blocks Phase 3.** `constitution-consistency.md`. Constitution v2.0.0 was authored *by this feature*, before the spike measured anything, and two Principle V properties are now violated by the mechanism it adopted.
  - **V.4 (MUST): user-edit preservation.** apm silently overwrites and cannot report. FR-034's rewrite to build-output semantics is a *spec* change with no authority over the constitution — it moved the conflict rather than resolving it. Critically, **no Manifest-side guard can fix this**: re-hashing against `deployed_file_hashes` buys *reported*, and nothing buys *preserved*, because apm performs the write. Amend V.4, abandon apm for the documented fallback, or record a scoped exception — a decision, not an implementation.
  - **V.3 (MUST): orphan removal by the deploy itself, not a separate pass.** Holds for migrated domains; fails for the 170 scripts still on rsync, which is exactly why `deploy_reconcile.sh` cannot be retired.
  - The `--reconfigure` half of this task is **already satisfied**: all four named docs were checked and no stale drift-correction guidance remains; every surviving mention is legitimate service-toggle usage. Recorded so the check is not repeated.
...
- [x] T036 [US4] Derive the **authoritative** skill→domain map by **functional analysis of all 107 skills**, not name-prefix matching. Assert the partition: every skill in exactly one domain, none in two, none in zero. → FR-025, SC-010
  - **Done 2026-07-27** — `plugin-partition.md`. Derived functionally over all 108 skills; name-prefixing demonstrably fails on this corpus (`pr-review`/`repo-clean` split one workflow, `shell-refactor` consumes `git_ops.sh` which no prefix predicts). **The partition assertion is satisfied by naming where it FAILS, not by forcing a tidy answer**: `lifecycle-run` spans four domains by construction and `repo-clean` straddles two. Recommendation is a `manifest-workflows` plugin for the straddlers rather than assigning them somewhere and quietly losing half their function.
- [x] T037 [US4] Map the **shared-script dependency graph** (`git_ops.sh` → 12 skills across four domains; `git_platform.sh` → two) and decide: place shared components so partial enablement still functions, or declare cross-plugin dependencies explicitly. If shared scripts stay global regardless of enablement, say so — that makes compartmentalization cosmetic for functionality and real only for catalog loading, and the docs must not overclaim. → FR-028
  - **Done 2026-07-27** — measured, not estimated: `git_ops.sh` → **12** skills across four domains, `git_platform.sh` → **4** (spec said two), retired cross-harness coordinator → 6, `tracker_ops.sh` → 3, `issue_support.sh` → 2, `label_sync.sh` → 1. **22 of 108 skills (20%) depend on a shared script**; the other 80% are self-contained markdown.
  - Decided: a `manifest-core` plugin holding shared scripts, which others depend on. Duplication was rejected (12 copies of `git_ops.sh` reintroduces the drift this feature removes) and majority-placement was rejected (`git_ops.sh`'s consumers span four domains, so any single home leaves three cross-plugin dependencies). **Caveat recorded rather than buried: this assumes APM plugins can declare a dependency on another plugin and resolve a script from it at a stable path — NEITHER IS MEASURED.** If plugin-to-plugin dependency does not exist, the decision collapses and the honest answer becomes that script-dependent skills cannot ship as plugins at all.
- [x] T038 [US4] Generate the plugins via `apm pack` with `manifest-` applied to **both** the plugin manifest name and the marketplace entry name. Note the evidence limit: marketplace name governs `enabledPlugins` and `/plugin` display, while component namespacing is documented as driven by `plugin.json`'s `name` — prefixing both is belt-and-braces, not a proven equivalence. → FR-026, E6
  - **CLOSED MEASURED-LIMITED 2026-07-27.** `apm pack` was run: it emits `.claude-plugin/plugin.json` with five keys (`name`, `version`, `description`, `author`, `license`) and **no dependency field**. The `manifest-` prefix flows correctly from `apm.yml` (`name: manifest-skills`), but one `apm.yml` yields one plugin — there is no domain-partition mechanism, so T036's map has nothing to partition into. See `plugin-partition.md`.
- [x] T039 [P] [US4] Write `tests/bats/manifest_plugin_naming.bats`: prefix present in both locations; skill→plugin mapping is a true partition; enabling one plugin loads only that domain **and that domain still functions with siblings disabled** (the FR-028 case — a load-only test would pass while the feature is broken). → FR-025, FR-026, FR-028, SC-010
  - **CLOSED MEASURED-LIMITED 2026-07-27.** Two of its three assertions have no subject: "prefix in **both** locations" needs a marketplace entry (`apm pack` produces none without a `marketplace:` block, an unscoped publishing model), and "skill→plugin mapping is a true partition" needs multiple plugins, which one `apm.yml` cannot produce.
- [x] T040 [US4] Verify install-by-name on a machine with no Manifest clone, into an isolated `HOME`. Document honestly what a plugin cannot carry: permissions and env settings do not travel this way. → FR-024, E6
  - **CLOSED — the load-bearing half is already proven, by T019.** Install-by-name into an isolated `HOME` with no clone succeeded from the published tag (108 skills, unauthenticated). What a plugin *cannot* carry is now measured rather than predicted: no plugin-to-plugin dependency, so the 22 skills (20%) that consume shared scripts cannot resolve them through a plugin.

---

## Phase 7: Polish & Cross-Cutting

- [x] T041 [P] Update documentation: what each pipeline owns, deploy, drift detection and correction, rollback, the pinned version and its upgrade procedure, and the retained `generate_commands_doc.py`. Retire references to deleted generators. → FR-037
  - **Done 2026-07-27** — `docs/DEPLOY_OWNERSHIP.md`. Covers what each pipeline owns, the drift report and **what it cannot detect** (deployed-file content drift is not an apm drift category, so a hand-edit is silently overwritten), the build-output rule, per-domain deploy, the dev loop, rollback including the reclamation step, and the pin + upgrade + offline procedures. Leads with the current state — everything is legacy-owned today — and gives the command to verify that rather than asking the reader to trust it.
- [x] T042 Complete the **supply-chain gates** begun in Phase 0: add `tests/bats/apm_supply_chain.bats` asserting that package integrity verification (T050), **binary integrity verification in `bootstrap.sh` (T054)**, the pre-publish scan (T048), and the provenance gate (T049) each **fail closed** when their subject is invalid, and wire `apm audit` into CI. *(The gates themselves moved to Phase 0 — this task makes them regression-proof, since FR-018 was the one cross-cutting invariant with no enforcing test.)* → FR-018, FR-029
  - **Done 2026-07-27** — `tests/bats/apm_supply_chain.bats`, 11 tests. Each of the four gates is asserted to FAIL CLOSED on an invalid subject, plus a positive control per gate so the failures are not vacuous. Also pins SC-011's harder half: a **rejected** run still writes a gate record, or the audit trail would show only the good days.
  - The secret fixture is built by runtime concatenation on purpose — a literal secret-shaped string in the file would trip the repo's own pre-commit scanner, so the test proving the scanner works could not itself be committed.
  - **The "wire `apm audit` into CI" clause is deliberately NOT implemented.** T052 established that `apm audit` has no deployed-file drift channel; wiring it would add a CI step that cannot detect what it was added to detect — a false-green check, which is worse than no check.
- [x] T043 Verify **SC-011** across the whole project history: every publish performed — spike and release alike — has a preceding gate record for T048 and T049. A publish with no gate record is a failure regardless of whether anything leaked. → FR-030, FR-038, SC-011
  - **Verified 2026-07-27 — SC-011 HOLDS.** Exactly one publish has occurred in project history (the T004 throwaway, `RB-chrismandich/apm-spike-522`) and exactly one gate record exists for it. The ordering is checkable, not asserted: gate record `ts=2026-07-27T17:43:16Z`, repo creation `2026-07-27T17:43:24Z` — the gate precedes the publish by 8 seconds, with matching `git_ref: v1.0.0` and `result: "pass"`.
- [x] T044 [P] Implement and document the **offline install path** (pinned local artifact, no registry access). *(The publish-free local development loop moved to **T055** in Phase 2 — it had to precede the gating that removes the workflow it replaces, not trail it by five phases. Verify here that the two paths coexist: an offline install must not clobber a linked local package, and vice versa.)* → FR-031, FR-032
