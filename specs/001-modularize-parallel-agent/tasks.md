...
description: "Task list for the retired cross-harness coordinator modularization into agents/ package"
---
...
- [x] T001 Capture baseline test results: run `pytest the retired coordinator integration test -v` and save output to `specs/001-modularize-parallel-agent/baseline-test-results.txt`
...
- [x] T004 Extract `Config` and `ServiceConfig` classes (lines 70–183 of `configs/claude/scripts/the retired cross-harness coordinator`) into `configs/claude/scripts/agents/config.py`; add stdlib-only imports (no cross-module deps)
...
- [x] T013 [US1] Replace `configs/claude/scripts/the retired cross-harness coordinator` with thin shim (≤15 lines): `sys.path.insert`, `from agents.cli import main`, `asyncio.run(main())` (depends on T012)
...
- [x] T015 [US1] Run `pytest the retired coordinator integration test -v` and confirm all tests that passed in T001 still pass (depends on T014)
...
- [x] T017 [P] [US2] Write `tests/python/agents/test_config.py`: migrate and adapt Config, ServiceConfig, Logger, and RateLimiter tests from `the retired coordinator integration test`; import from `agents.config`
...
- [x] T023 [US2] Update `the retired coordinator integration test` imports: replace `from parallel_agent import (Config, ...)` with per-module imports `from agents.config import Config, ServiceConfig, Logger, RateLimiter` etc. (FR-007)
...
- [x] T027 [P] Verify behavioral equivalence: compare CLI `--help` output before and after (`python configs/claude/scripts/the retired cross-harness coordinator --help`) — all flags must be present (SC-003)
...
- [x] T029 [P] Remove dead code: confirm the original monolith body in `the retired cross-harness coordinator` is fully replaced by the shim (no old class definitions remain in the entry-point file)
- [ ] T030 **[REQUIRED MERGE GATE]** Constitution Principle II: run parallel agent cross-verification before merging — `~/.claude/scripts/the retired cross-harness coordinator --validate --review configs/claude/scripts/agents/`; this is a MUST per the constitution for >200-line modifications; PR MUST NOT merge without a passing review at ≥50% consensus
...
4. **STOP and VALIDATE**: Run `pytest the retired coordinator integration test -v` and verify `python the retired cross-harness coordinator --help` shows all flags
