...
- [X] T038 [US5] Implement review-gate verdict parsing in `lifecycle.sh` (spec-review `--format json`: []→APPROVED, findings→NEEDS_REVIEW/BLOCKED by severity; optional `the retired cross-harness coordinator --validate` consensus) → `decide` gate_type=verdict (FR-027)
