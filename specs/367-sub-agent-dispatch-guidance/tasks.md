...
- [X] T003 Author the shared **Sub-Agent Selection Rules** in a NEW read-on-demand reference `configs/claude/references/sub-agent-dispatch.md` (NOT inline in the auto-loaded CLAUDE.md — context_budget.bats allows only ~537 bytes headroom there). Content per research R2/R8 + data-model "Shared Selection Rules": native Task/Agent sub-agents vs `the retired cross-harness coordinator`; cross-platform fallback (Claude-only Task tool → inline or `the retired cross-harness coordinator` on Cursor/Gemini/Codex/Antigravity); no-recursion rule; ≥3-independent-unit default floor. Then add ONE pointer line to the "Reference Index" in `configs/claude/CLAUDE.md`.
...
**Goal**: Every dispatching skill points to the right mechanism (native Task vs `the retired cross-harness coordinator`)
...
verification to `the retired cross-harness coordinator`.

- [X] T017 [US2] In `configs/claude/references/sub-agent-dispatch.md` selection-rules section, add the explicit mechanism-by-task-type table (parallel reads/research → native Task; independent cross-model verification of security/architecture/>200-line changes → `the retired cross-harness coordinator`; <3 units → inline) and the non-Claude fallback row.
- [X] T018 [US2] Ensure the `refactor-*` skills keep `retired_cross_harness_coordinator_policy: always` (external cross-verification, constitution Tier-1) AND carry their native `subagents` trigger — verify the two fields coexist without contradiction in `command_config.yml`.
