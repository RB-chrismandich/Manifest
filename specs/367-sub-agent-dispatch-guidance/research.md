...
- **Rationale**: `tool_policies` already encodes per-skill `retired_cross_harness_coordinator_policy: always|conditional|never`
...
- **Decision**: Author the native-Task-subagent vs `the retired cross-harness coordinator` selection rules (with the
...
- **Rationale**: Sub-agent dispatch has fixed overhead; a uniform floor prevents fan-out on trivial
  ...
  with what `retired_cross_harness_coordinator_policy: conditional` skills already use.
...
- **Decision**: The shared selection rules MUST state: native Task/Agent sub-agents are Claude-only;
  on Cursor/Gemini/Codex/Antigravity, skills either use `the retired cross-harness coordinator` (cross-platform) or run
