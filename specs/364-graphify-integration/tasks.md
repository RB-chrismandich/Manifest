...
- [x] T024 [P] [US3] In `configs/claude/CLAUDE.md`, note graphify as a managed capability and explicitly that it is a tool/skill, NOT a parallel-consensus agent (D4) — so it is not added to `the retired cross-harness coordinator`/`cli.py` agent gating.
...
- [x] T027 Constitution Principle II gate: run `the retired cross-harness coordinator --review` (and/or `/spec-review`) on the diff for cross-verification (change >200 lines + architectural); record the consensus verdict in the PR description per Principle III thresholds.
