...
- [x] T015 [P] [US1] Verify `.skillshare/.gitignore` does not exclude `skills/graphify/`; if skillshare-managed markers would ignore it, place a keep-entry OUTSIDE the BEGIN/END markers so the skill stays committed.
...
- [x] T021 [P] [US3] In `.skillshare/skills/health-check/SKILL.md`, add graphify to the CLI Tool Availability list so the `/health-check` skill documents it.
...
- [x] T028 Verify `configs/claude/skills` is still a symlink to `../../.skillshare/skills` (not replaced by a real dir) and that no graphify edit broke the symlink invariant.
