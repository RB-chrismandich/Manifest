...
- Q: How should the `/graphify` skill be delivered to the AI assistants? → A: Vendor the skill into the project's single skill source of truth (`.retired skill supply/skills/`) and deploy it via the existing cross-assistant pipeline; do **not** use graphify's own installer for skill placement (avoids bypassing the source of truth and the FR-010 collision).
...
- **FR-009**: The graphify skill MUST be vendored into the project's single skill source of truth (`.retired skill supply/skills/`) and deployed through the existing cross-assistant deployment path, so it stays consistent across all assistant targets and is not hand-placed per assistant. The system MUST NOT rely on graphify's own installer to place the skill (which would bypass the source of truth and risk the FR-010 collision).
...
- **Skill artifact**: The `/graphify` skill definition vendored into the project's single skill source of truth (`.retired skill supply/skills/`) and deployed to each enabled assistant.
