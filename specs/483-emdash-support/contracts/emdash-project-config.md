...
```jsonc
{
  ...
  "preservePatterns": ["guidance_local.yml", ".env"],

  "scripts": {
    // Runs once when emdash creates the worktree. Must be idempotent + fail closed.
    "setup": "git submodule update --init --recursive && uv sync",
...
3. **Secrets stay untracked** — files referenced for preservation that hold secrets (`.env`) MUST remain gitignored; the repo never commits them (FR-006 AC3).
