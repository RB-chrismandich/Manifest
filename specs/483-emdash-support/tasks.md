...
- [x] T008 [P] [US2] Author repo-root `.emdash.json` per `contracts/emdash-project-config.md`: `preservePatterns` = `[]` (this repo has no in-tree untracked config — `guidance_local.yml` is gitignored but lives HOME-side at `~/.claude/config/`, never inside the repo tree, so it isn't a valid preserve target; NOT `.env` either — this repo has none and it is not gitignored here, spec-review F2); `scripts.setup` = `git submodule update --init --recursive` (bats helpers) + `pip install -r tests/requirements-ci.txt` (matches CI; NOT `uv sync` — this repo doesn't use uv for deps, spec-review F1), idempotent and fail-closed; `shellSetup` minimal. [F1][F2]
