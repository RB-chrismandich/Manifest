...
### User Story 3 - Same loop over a design-doc doc pair (Priority: P3)

A maintainer working in the design-doc layout (a design doc plus a matching implementation plan, with tasks embedded in the plan rather than in a separate file) points the same command at their design doc. The loop resolves the paired artifacts, and everything else behaves identically to the speckit case.
...
**Independent Test**: Run the loop against a fixture design-doc design doc with a paired plan. Verify the run resolves both artifacts, completes the same phases, and never reports a missing tasks artifact.

**Acceptance Scenarios**:

1. **Given** a design-doc design doc and a paired plan, **When** the operator invokes the loop with the design doc path, **Then** the loop resolves spec and plan per the repo's artifact-discovery precedence and runs identically to the speckit case.
2. **Given** a design-doc-layout run, **When** the run produces any report or error, **Then** it never reports a missing tasks artifact.
...
- **Layout ambiguity**: A target could plausibly match both layouts → the repo's existing discovery precedence (explicit paths > speckit > design-doc) decides, and the resolved choice is recorded in the run log (per FR-001).
...
- **FR-001**: The feature MUST provide a single entry point that accepts one target path and supports both speckit feature directories and design-doc design docs, resolving the spec and plan artifacts according to the repo's existing spec-artifact discovery contract (explicit paths win, then speckit detection, then design-doc fallback). It MUST NOT introduce a second, divergent discovery mechanism.
- **FR-002**: The loop MUST NOT require a tasks artifact: design-doc runs (where tasks are embedded in the plan) MUST never be reported as missing tasks, and speckit runs MUST work with spec and plan alone.
...
- **SC-001**: An operator can start a run on either supported layout with a single command and zero manual context assembly (no copying spec/plan content by hand), in both a speckit and a design-doc fixture.
