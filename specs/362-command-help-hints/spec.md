...
- Q: Feature scope boundary — read-only guidance, or also command management? → A: Read-only guidance only (discover/search/hint/remind); creating, editing, enabling, or disabling commands is out of scope (owned by skillshare / sync-skills / services.yml).
...
- **Out of scope (v1)**: command *management* — creating, editing, enabling, or disabling commands — is explicitly excluded (per Clarifications 2026-06-21). Those actions remain owned by `skillshare`, `sync-skills`, and `services.yml`. This feature only *surfaces* and *guides toward* existing commands (read-only).
