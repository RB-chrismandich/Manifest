...
**Deciders**: repo owner · **Context**: PR #346 (issue-orchestrator) vs merged PR #354 (auto-issue-dev)
...
- **auto-issue-dev (#354, merged):** a skill + `auto_issue_dev.sh` + `/loop`;
  runs in the Claude Code session; TDD via `superpowers`; `/verify`; allowlist
  `auto-dev` label.
...
| Dimension (long-term) | auto-issue-dev (#354) | orchestrator daemon (#346) |
|---|---|---|
...
| Autonomous-output safety | Thin (`/verify` only) | Strong (consensus + Tier 1/2 gates) |
