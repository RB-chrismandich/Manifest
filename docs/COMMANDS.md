...
_115 commands, generated from `.apm/skills/*/SKILL.md`._
...
| `/security-review-diff` | Use when asked to review a change or diff for security vulnerabilities — applies a disciplined source-to-sink method that reports only real findings, not robustness or best-practice nits. To verify/refute an existing candidate list, use security-refute-findings. | Use when asked to review a change or diff for security vulnerabilities — applies a disciplined source-to-sink method that reports only real findings, not robustness or best-practice nits. | available |
...
| `/spec-review` | Use before finalizing spec/plan/tasks — cross-reference for internal consistency via the parallel-agent panel (excluding the author), deduped findings. Analysis-only, never edits. Speckit/superpowers; auto-discovers or explicit paths. | Use before finalizing spec/plan/tasks — cross-reference for internal consistency via the parallel-agent panel (excluding the author), deduped findings. | available |
...
| `/skill-evolve` | Use after a stretch of work to harvest SkillClaw-evolved skills from Claude Code session transcripts into a reviewed PR against .apm/skills/. Dry-run by default; requires --enable-skillclaw and claude CLI login. Never writes source of truth directly. | Use after a stretch of work to harvest SkillClaw-evolved skills from Claude Code session transcripts into a reviewed PR against . | available |
...
| `/config-audit` | Verify cross-platform config drift and symlink integrity across Claude, Cursor, Gemini, Codex, and Antigravity. | Verify cross-platform config drift and symlink integrity across Claude, Cursor, Gemini, Codex, and Antigravity. | available |
...
| Command | Description | When to use | Status |
|---------|-------------|-------------|--------|
| `/antipattern-detect` | Use when lint/test failures, security findings, or repeated (3+) review feedback surface a recurring pattern worth recording in the knowledge base. | Use when lint/test failures, security findings, or repeated (3+) review feedback surface a recurring pattern worth recording in the knowledge base. | available |
...
| `/env-check` | Diagnoses Manifest environment health — CLI tools, auth, config syntax, MCP, symlinks — when setup seems broken or post-bootstrap. | Diagnoses Manifest environment health — CLI tools, auth, config syntax, MCP, symlinks — when setup seems broken or post-bootstrap. | available |
...
| `/learning-capture` | Use after a significant task, debug session, or review to capture a lesson (pattern, antipattern, tool-discovery, config-insight) in knowledge_base.yml, or query/list/stats existing entries. | Use after a significant task, debug session, or review to capture a lesson (pattern, antipattern, tool-discovery, config-insight) in knowledge_base. | available |
...
| `/metrics-report` | Use when the user wants agent-efficiency metrics — completion rates, error patterns, consensus scores, model usage — visualized from ~/.claude/.agent_outputs/ logs as markdown dashboard tables ("agent dashboard", "metrics report"). | Use when the user wants agent-efficiency metrics — completion rates, error patterns, consensus scores, model usage — visualized from ~/. | available |
...
| `/test-isolate-ambient` | Use before verifying Manifest deploy, hook, or installer behaviour — a pass against your real ~/.claude, settings.json, or checked-out repo proves nothing; the ambient state IS the fixture. Provides isolation handles (TARGET_DIR, ISSUE_HOOKS_SETTINGS, throwaway clone) and the rule for making the varying thing vary. | Use before verifying Manifest deploy, hook, or installer behaviour — a pass against your real ~/. | available |
