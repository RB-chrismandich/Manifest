...
| Command | Description | When to use | Status |
|---------|-------------|-------------|--------|
| `/docs-all` | Run docs-improve-readme, docs-generate-diagrams, and docs-improve in one pass as sub-agents, bracketed by a docs_lint.py measurement so the report shows a real line-count delta. Use to refresh the whole doc set at once. | Run docs-improve-readme, docs-generate-diagrams, and docs-improve in one pass as sub-agents, bracketed by a docs_lint. | available |
...
| `/docs-improve` | Audit and improve a project's docs set using Diataxis — enforce per-type line caps, split over-cap pages into a hub plus sub-pages, and cut fluff. Use for "improve the docs", "the docs are too long", "audit our documentation". | Audit and improve a project's docs set using Diataxis — enforce per-type line caps, split over-cap pages into a hub plus sub-pages, and cut fluff. | available |
...
| `/skill-evolve` | Turn SkillClaw-evolved skills into a reviewed PR into .apm/skills/. Dry-run by default; --apply opens one PR with one commit per skill. Requires --enable-skillclaw and claude CLI login. Never writes source of truth directly — all changes go through PR review. | Turn SkillClaw-evolved skills into a reviewed PR into . | available |
...
| `/docker-compose-commandments` | Audit or harden a docker-compose.yaml against DC-001..DC-010 — pinning, secrets, healthchecks, limits, network isolation, volumes, non-root, logs. Use for "review my compose file" or a DC-NNN finding. | Audit or harden a docker-compose. | available |
...
| `/false-green-check-audit` | Use when writing or reviewing a health-check, status, validation, or CI gate that can SKIP verifying something (missing credential, unsupported provider, absent tool) — ensure a skipped check never renders as a green pass, and verify through the channel the user actually uses. | Use when writing or reviewing a health-check, status, validation, or CI gate that can SKIP verifying something (missing credential, unsupported provider, absent tool) — ensure a skipped check never renders as a green pass, and verify through the channel the user actually uses. | available |
...
| `/spec-amend` | Record design-pass findings that are upstream spec gaps, then land them as normative spec edits. | Record design-pass findings that are upstream spec gaps, then land them as normative spec edits. | available |
...
| `/test-isolate-ambient` | Use before verifying Manifest deploy, hook, or installer behaviour — a pass against your real ~/.claude, settings.json, or the checked-out repo proves nothing, because the ambient state IS the fixture. Isolation handles plus the rule for making the varying thing vary. | Use before verifying Manifest deploy, hook, or installer behaviour — a pass against your real ~/. | available |
