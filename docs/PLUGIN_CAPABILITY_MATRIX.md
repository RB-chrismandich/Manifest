...
| `manifest-workspace:hook:manifest-hooks` | contract hook | READY | DEGRADED(The advisory manifest-hooks catalog is not a Codex lifecycle hook registration. Codex lifecycle support is declared per event below.) | DEGRADED(Gemini extensions cannot install the bundle's advisory manifest-hooks catalog as native BeforeTool hooks; use ai-hooks-integration for explicit installation.) | READY | DEGRADED(Antigravity has no native lifecycle hook surface.) | DEGRADED(Devin has no native lifecycle hook surface.) |
