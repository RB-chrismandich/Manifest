...
| `manifest-workspace:guidance:workspace-token-economy` | contract guidance | N/A(Claude Code's plugin.json has no context-injection field; delivered instead via the workspace-token-economy-context SessionStart hook.) | N/A(Codex has no plugin guidance-file surface; delivered instead via the workspace-token-economy-context SessionStart hook.) | READY | N/A(Cursor's plugin adapter indexes bundles without inventing an activation mechanism (see CursorAdapter); Cursor instead receives this guidance from the bootstrap-deployed configs/cursor/rules/orchestration.mdc.) | READY | N/A(MIGRATION BLOCKER — Devin single-owns ~/.codeium/windsurf/memories/global_rules.md exclusively for the manifest-i-have-adhd bundle (the Devin adapter's _prepare_adhd_rule guard and the deploy_devin_adhd_rule step that writes it). Sharing that file across bundles requires extending the Devin adapter's owned-file ownership model, which is out of scope here; /token-conserve remains available for Devin users until that adapter work ships.) |
