...
| `manifest-workspace:hook:claude-compaction-continuity` | contract hook | READY | N/A(Codex exposes no verified completed-compaction lifecycle sequence.) | N/A(Gemini exposes no verified completed-compaction lifecycle sequence.) | N/A(Cursor exposes no verified completed-compaction lifecycle sequence.) | N/A(Antigravity exposes no verified completed-compaction lifecycle sequence.) | N/A(Devin exposes no verified completed-compaction lifecycle sequence.) |
...
| `manifest-workspace:hook:workspace-token-economy-context` | contract hook | READY | READY | N/A(Gemini already receives this guidance through the guidance component's generated contextFileName; a duplicate SessionStart hook is not a verified Gemini extension surface.) | N/A(Cursor's hook events (beforeShellExecution etc.) have no SessionStart/always-on-context equivalent and CursorAdapter deliberately indexes bundles without inventing an activation mechanism; Cursor instead receives this guidance from the bootstrap-deployed configs/cursor/rules/orchestration.mdc.) | N/A(Antigravity already receives this guidance through the guidance component's imported contextFileName; a duplicate SessionStart hook is not a verified Antigravity surface.) | N/A(Devin has no SessionStart-equivalent hook execution surface.) |
...
| `manifest-workspace:guidance:workspace-token-economy` | contract guidance | N/A(Claude Code's plugin.json has no context-injection field; delivered instead via the workspace-token-economy-context SessionStart hook.) | N/A(Codex has no plugin guidance-file surface; delivered instead via the workspace-token-economy-context SessionStart hook.) | READY | N/A(Cursor's plugin adapter indexes bundles without inventing an activation mechanism (see CursorAdapter); Cursor instead receives this guidance from the bootstrap-deployed configs/cursor/rules/orchestration.mdc.) | READY | N/A(MIGRATION BLOCKER — Devin single-owns ~/.codeium/windsurf/memories/global_rules.md exclusively for the manifest-i-have-adhd bundle (src/manifest_agent/adapters/devin.py::_prepare_adhd_rule, bootstrap/lib/deploy.sh::deploy_devin_adhd_rule). Sharing that file across bundles requires extending the Devin adapter's owned-file ownership model, which is out of scope here; /token-conserve remains available for Devin users until that adapter work ships.) |
