...
It returns `VALID`, `INVALID`, or `ERROR`, with diagnostics containing a code,
field path, and explanation. `ERROR` covers exit 3 (input unavailable or an
operational error); `INVALID` covers exit 2 (structurally invalid content);
`VALID` covers exit 0. Operational errors never produce a valid receipt. JSON
output remains machine-readable; diagnostics must not echo source documents,
credentials, or entire input strings.
...
comparison for removed or weakened requirements, boundaries (exclusions,
preserved behavior, approval-required actions), and output obligations. Hashes establish
...
plugins/manifest-spec-planning/skills/goal-compose/
  SKILL.md
  agents/openai.yaml           # generated by tools/generate_plugin_views.py, not hand-authored
...
| FR-10 | Scope survives revision and handoff | Requirement deletion, weakened criterion, removed/weakened boundary or output obligation, changed revision and stale checkpoint scenarios |
