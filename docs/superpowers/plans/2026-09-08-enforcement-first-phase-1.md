...
class RegressionContract(unittest.TestCase):
    def setUp(self):
        ...
        for directory in (self.repo, self.bin, self.home, self.logs):
            directory.mkdir()
        requested_bash = os.environ.get("PR_SMOKE_TEST_BASH", "bash")
        self.runner_bash = shutil.which(requested_bash)
        self.assertIsNotNone(
            self.runner_bash, f"required fixture tool: {requested_bash}"
        )
        # Resolve the trusted interpreter before restricting PATH, then expose
        # only that binary to checks which select the default `bash`.
        (self.bin / "bash").symlink_to(self.runner_bash)
...
```python
    ...
    def test_quick_does_not_probe_unselected_syntax_interpreter(self):
        result = self.run_runner(
            "--quick", extra_env={"PR_SMOKE_BASH": "missing-bash"}
        )
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertNotIn("shell syntax", result.stdout)

    def test_no_scripts_does_not_probe_unselected_syntax_interpreter(self):
        self.git("rm", "-f", "scripts/generate_commands_doc.py")
        result = self.run_runner(extra_env={"PR_SMOKE_BASH": "missing-bash"})
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertNotIn("shell syntax", result.stdout)
...
- [ ] Replace the existing full-mode discovered-script branch's `run_gate 'shell syntax' bash -n ...` call with the selected-interpreter flow below.
...
Initialize and probe the explicitly selectable syntax interpreter only within the
existing full-mode branch after `manifest_scripts_dir` is nonempty. The probe
and `check_shell_syntax` are both omitted when quick mode is selected or when
the command-generator discovery did not select Manifest scripts:

```bash
if [[ "$QUICK" -eq 0 && -n "$manifest_scripts_dir" ]]; then
...
A requested but unavailable interpreter is BLOCKED only after shell syntax is
selected. The runner itself is tested under the interpreter selected by
`PR_SMOKE_TEST_BASH`; selecting it only for `-n` would not prove that the
runner executes on Bash 3.2.
