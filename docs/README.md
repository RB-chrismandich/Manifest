# Documentation

> Index of every Manifest documentation page. One line each.

## Start here

- **New?** → [Getting Started](GETTING_STARTED.md)
- **Something broke?** → [Troubleshooting](troubleshooting/README.md)
- **Changing a setting?** → [Configuration](configuration/README.md)
- **Understanding the system?** → [Architecture Diagrams](diagrams/README.md)

## All documents

| Page | Answers |
|------|---------|
| [GETTING_STARTED.md](GETTING_STARTED.md) | Install, first run, first command |
| [FEATURES.md](FEATURES.md) | What Manifest does |
| [COMMANDS.md](COMMANDS.md) | Command surface and how to author one |
| [configuration/](configuration/README.md) | Every setting, by file |
| [troubleshooting/](troubleshooting/README.md) | Problems, indexed by symptom |
| [diagrams/](diagrams/README.md) | 20 Mermaid diagrams, by subject |
| [model-policy/](../docs/MODEL-POLICY.md) | Which model runs what |
| [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) | Repository layout |
| [TESTING.md](TESTING.md) | Running the test suites |
| [CODING_STANDARDS.md](CODING_STANDARDS.md) | Per-language standards |
| [SKILL-NAMING.md](SKILL-NAMING.md) | Naming rules for skills |
| [SPEC-SYSTEMS.md](SPEC-SYSTEMS.md) | speckit vs superpowers vs .plans |
| [DEPLOY_OWNERSHIP.md](DEPLOY_OWNERSHIP.md) | Who owns which deployed path |
| [PLUGIN_RELEASE.md](PLUGIN_RELEASE.md) | Releasing plugin bundles |
| [PLUGIN_CAPABILITY_MATRIX.md](PLUGIN_CAPABILITY_MATRIX.md) | Bundle capabilities |
| [APM_PINNING.md](APM_PINNING.md) | Pinning the apm skill package |
| [PRE_COMMIT.md](PRE_COMMIT.md) | Hook configuration |
| [SKILLCLAW.md](SKILLCLAW.md) | Optional session capture and evolve |
| [EMDASH.md](EMDASH.md) | Working in emdash worktrees |
| [METRICS.md](METRICS.md) | Agent result metrics |
| [TOKEN_BENCHMARK.md](TOKEN_BENCHMARK.md) | Measured context overhead |
| [KNOWLEDGE_BASE.md](KNOWLEDGE_BASE.md) | Antipattern registry |

## About these docs

| Page | Answers |
|------|---------|
| [NAVIGATION.md](NAVIGATION.md) | Which pages suit which audience |
| [DOC_NAVIGATION.md](DOC_NAVIGATION.md) | Reading paths through the set |
| [DOC_STANDARDS.md](DOC_STANDARDS.md) | Diataxis types and line caps |
| [DOC_CONTRIBUTING.md](DOC_CONTRIBUTING.md) | How to add or change a page |
| [DOC_HEALTH.md](DOC_HEALTH.md) | Dated coverage record |

## Related Resources

- [../README.md](../README.md) — project overview
- [../CLAUDE.md](../CLAUDE.md) — repository guide for Claude Code
- [../AGENTS.md](../AGENTS.md) — agent instructions
