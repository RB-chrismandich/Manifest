...
5. **Harden the probe loop.** Redirect `</dev/null` so the probed CLI doesn't drain the iteration's stdin and check only the first item (see `shell-audit-errexit`).
