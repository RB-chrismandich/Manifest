...
2. **Check which chain the ACL targets.** Grep the deploy/provisioning script for `iptables`/`nft` rules guarding that
   ...
   Verify the RETURN/ACCEPT for the allowlisted source is inserted **above** the DROP (rule order matters; `-I`
   prepends).

3. **Verify the control is testable, not just present.** A single-`/32` source-IP filter over plaintext HTTP is
