...
1. **Map the stack first.** `for n in <PRs>; do ~/.claude/scripts/git_ops.sh pr-view $n --json number,baseRefName,headRefName; done`. Confirm the chain: A(base `main`) ← B(base A) ← C(base B) …
...
3. **Merge bottom-up, one at a time.** For each parent: wait for green + `MERGEABLE`, then `~/.claude/scripts/git_ops.sh pr-merge <parent> --merge` **without** `--delete-branch`.
4. **Immediately retarget the child** onto the surviving base: `~/.claude/scripts/git_ops.sh pr-edit <child> --base main`; verify with `~/.claude/scripts/git_ops.sh pr-view <child> --json baseRefName`.
...
6. **Recover a cascaded-closed child.** If you already deleted a base and GitHub auto-closed the child (a closed PR can't be retargeted or reopened while its base ref is gone): restore the ref with `git push origin <merged-sha>:refs/heads/<deleted-base>`, then `~/.claude/scripts/git_ops.sh pr-reopen <child>`, `~/.claude/scripts/git_ops.sh pr-edit <child> --base main`, then delete the temp ref.
