...
1. **Map the stack first.** `for n in <PRs>; do gh pr view $n --json number,baseRefName,headRefName; done`. Confirm the
...
3. **Merge bottom-up, one at a time.** For each parent: wait for green + `MERGEABLE`, then `gh pr merge <parent>
   --merge` **without** `--delete-branch`.
4. **Immediately retarget the child** onto the surviving base: `gh pr edit <child> --base main`; verify with `gh pr view
   <child> --json baseRefName`.
...
6. **Recover a cascaded-closed child.** If you already deleted a base and GitHub auto-closed the child (a closed PR
   ...
   <merged-sha>:refs/heads/<deleted-base>`, then `gh pr reopen <child>`, `gh pr edit <child> --base main`, then delete
   the temp ref.
