...
# --- apm (Agent Package Manager) — T054 / FR-029 -----------------------------
#
# apm writes hooks and MCP server definitions into five home trees. Hooks
# execute shell commands and MCP entries spawn long-lived processes with tool
# access, so this is code execution, not inert configuration — the tool's own
# install channel is part of the trust surface.
#
# Therefore this function FAILS CLOSED at every step: no fallback to an
# unverified binary, no "warn and continue", and "indeterminate" (cannot
# compute a checksum) counts as failure. That is the deliberate difference from
# install_graphify, which is an optional capability and is allowed to degrade.
# Distinct from apm_install_verify.sh (T050/FR-018), which verifies *packages*
# at install time; this verifies the tool doing the installing.
#
# The version pin and digest come from T001
# (specs/522-apm-deploy-migration/decision-record.md). PyPI provenance does not
# tie the artifact to microsoft/apm — apm-cli 0.26.0 ships no home_page, no
# project_urls and no author — so the digest recorded at spike time *is* the
# provenance. Bump the two together; a version bump without a re-recorded
# digest silently disables the only check there is.
APM_PINNED_VERSION="${APM_PINNED_VERSION:-0.26.0}"
APM_WHEEL_SHA256="${APM_WHEEL_SHA256:-76290c42a9f9412e3ae8ea98d20acbdfbf4bee6dc32e951f616feab9bc82e7ad}"
APM_PYPI_INDEX="${APM_PYPI_INDEX:-https://pypi.org/pypi}"

# Resolve the pinned version's py3-none-any wheel URL from the PyPI JSON API.
# Deliberately does NOT use `command_exists` — a stubbed capability probe must
# not be able to turn the integrity check into a no-op.
apm_resolve_wheel_url() {
    local py="${PYTHON_CMD:-python3}"
    command -v curl > /dev/null 2>&1 || return 1
    command -v "$py" > /dev/null 2>&1 || return 1
    curl -fsSL "$APM_PYPI_INDEX/apm-cli/$APM_PINNED_VERSION/json" 2> /dev/null | "$py" -c '
import json, sys
try:
...
# Empty output on any failure, which the caller treats as a rejection.
apm_sha256() {
    local file="$1"
...
# Probe the installed binary's version with $HOME redirected at a throwaway
# directory. T001 finding 4: `apm --help` alone creates ~/.apm/config.json, so
# apm is not side-effect-free at any invocation — verifying the install must
# not be the thing that provisions a user's home.
apm_installed_version() {
    local bin probe_home version
    bin="$(apm_binary)"
    [[ -n "$bin" ]] || return 1
    probe_home="$(mktemp -d)" || return 1
    version="$(HOME="$probe_home" "$bin" --version 2> /dev/null | sed -n 's/.*version \([0-9][0-9.]*\).*/\1/p')"
    rm -rf "$probe_home"
    [[ -n "$version" ]] || return 1
    echo "$version"
}
...
install_apm_cli() {
    if [[ "${ENABLE_APM:-false}" != true ]]; then
        print_info "apm is disabled - skipping installation"
        return 0
    fi

    print_step "Checking for apm (Agent Package Manager) v$APM_PINNED_VERSION..."
    ...
    if command_exists uv; then
        ...
        print_error "uv is required to install apm and was not found — refusing to install apm by any other route"
        return 1
    fi

    # Idempotent only on an exact version match. A different installed version
    # is replaced, not accepted: the digest below pins one specific artifact,
    # so "some apm is present" is not the property being asserted.
    local pin_re="${APM_PINNED_VERSION//./\\.}"
    if "$uv_bin" tool list 2> /dev/null | grep -qE "^apm-cli v${pin_re}( |$)"; then
        print_success "apm v$APM_PINNED_VERSION is already installed"
        return 0
    fi

    local tmp
    tmp="$(mktemp -d)" || {
        print_error "Could not create a temporary directory for the apm download"
        return 1
    }
    ...
    local actual
    actual="$(apm_sha256 "$wheel")" || actual=""
    if [[ -z "$actual" ]]; then
        print_error "Could not compute a checksum for the downloaded apm wheel — refusing to install an unverified binary"
        rm -rf "$tmp"
        return 1
    fi
    if [[ "$actual" != "$APM_WHEEL_SHA256" ]]; then
        print_error "apm wheel checksum mismatch: expected $APM_WHEEL_SHA256, got $actual — refusing to install"
        rm -rf "$tmp"
        return 1
    fi
    print_success "apm wheel checksum verified"

    # Install the VERIFIED BYTES on disk, never the package name. Re-resolving
    # `apm-cli==$VERSION` here would fetch a second time and discard the
    # verification that just happened.
    if ! "$uv_bin" tool install --force "$wheel"; then
        print_error "uv failed to install the verified apm wheel"
        rm -rf "$tmp"
        return 1
    fi
    rm -rf "$tmp"
    ...
    local got
    got="$(apm_installed_version)" || got=""
    if [[ "$got" != "$APM_PINNED_VERSION" ]]; then
        print_error "Installed apm reports version '$got', expected '$APM_PINNED_VERSION' — treating as a failed install"
        return 1
    fi

    print_success "apm v$APM_PINNED_VERSION installed and verified"
    return 0
}
