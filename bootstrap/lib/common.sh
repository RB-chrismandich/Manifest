...
run_with_spinner() {
    ...
    (
        ...
        local tmp_log
        tmp_log=$(mktemp "${TMPDIR:-/tmp}/log_XXXXXX")
        trap 'tput cnorm 2>/dev/null || true; rm -f "$tmp_log"; trap - INT TERM; kill -s TERM $$' TERM
        trap 'tput cnorm 2>/dev/null || true; rm -f "$tmp_log"; trap - INT TERM; kill -s INT $$' INT
