...
run_with_spinner() {
    ...
    (
        ...
        local log_file
        log_file=$(mktemp "${TMPDIR:-/tmp}/log_XXXXXX")

        eval "$cmd" > "$log_file" 2>&1 &
        local pid=$!

        trap 'tput cnorm 2>/dev/null || true; rm -f "$log_file"; kill -9 "$pid" 2>/dev/null || true; kill -s TERM $$; exit 130' INT TERM

        local spin=("⠋" "⠙" "⠹" "⠸" "⠼" "⠴" "⠦" "⠧" "⠇" "⠏")
        local i=0

        while kill -0 "$pid" 2> /dev/null; do
            printf "\r%b%s%b %s..." "${CYAN}" "${spin[i]}" "${NC}" "$msg"
            ((i = (i + 1) % 10)) || true
            sleep 0.1
        done
