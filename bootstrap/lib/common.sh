...
run_with_spinner() {
    ...
    # Use a subshell to isolate traps and cursor state
    (
        local log_file
        log_file=$(mktemp "${TMPDIR:-/tmp}/spin_XXXXXX") || exit 1

        local pid
        eval "$cmd" > "$log_file" 2>&1 &
        pid=$!

        trap 'tput cnorm 2>/dev/null || true; rm -f "'"$log_file"'"; kill -s TERM '"$pid"' 2>/dev/null || true; exit 130' INT TERM
