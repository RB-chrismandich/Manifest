...
try:
    ...
    completed = subprocess.run(command, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True, timeout=25, check=False)
    if completed.returncode != 0:
        detail = diagnostic(completed.stderr)
        raise RuntimeError(f'Chromium exited {completed.returncode}{": " + detail if detail else ""}')
