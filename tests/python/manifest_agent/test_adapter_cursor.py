...
def marketplace_json(
    ...
    *,
    ...
    plugins: Sequence[str] = DOMAIN_BUNDLES,
) -> str:
    return json.dumps(
        [
            {
                ...
                "plugins": [
                    {
                        "name": name,
                        "version": "0.2.0",
                        "skills": [f"skills/skill-{name}/SKILL.md"],
                    }
                    for name in plugins
                ],
            }
...
def test_cursor_indexes_immutable_marketplace_ref(desired: DesiredState) -> None:
    runner = QueueRunner([command(), command(stdout=marketplace_json(desired))])
...
def test_cursor_inspect_requires_documented_nine_plugin_inventory(
    desired: DesiredState,
) -> None:
    runner = QueueRunner(
        [command(stdout=marketplace_json(desired, plugins=DOMAIN_BUNDLES[:-1]))]
    )

    result = CursorAdapter(runner=runner, which=lambda name: name).inspect(desired)

    assert result.state is ResultState.BLOCKED
    assert DOMAIN_BUNDLES[-1] in " ".join(result.errors)
...
def test_cursor_uninstall_removes_only_receipt_owned_marketplace_url(
    ...
    receipt = HarnessReceipt(
        ...
        owned_entries=(
            OwnedEntry("marketplace", desired.repository_url, "receipt"),
    ...
    result = CursorAdapter(runner=runner, which=lambda name: name).uninstall(receipt)
