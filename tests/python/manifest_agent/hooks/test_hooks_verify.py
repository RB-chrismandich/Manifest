...
@pytest.mark.parametrize(
    ("version_exit", "protocol_exit"),
    ((7, 0), (0, 7)),
)
def test_verify_rejects_nonzero_probe_exit(verify_env, version_exit, protocol_exit):
    env = verify_env
    _write_fake_client(
        env["bin_dir"],
        version="2.5.1",
    ...
    _write_fixture(env["repo_root"], env["fixture_dir"], SHAPE)
    entry = verify.load_matrix(env["matrix_path"])["claude_code"]

    result = verify.verify_client("claude_code", entry, _config(env))
...
def test_verify_refuses_to_promote_when_any_fixture_was_unverified(verify_env):
    env = verify_env
    _write_fake_client(env["bin_dir"], version="3.1.0", shape=SHAPE)
    _write_matrix(
        env["matrix_path"],
        executable_name="fake-client",
        protocol_probe=True,
    ...
    _write_fixture(env["repo_root"], env["fixture_dir"], SHAPE)
    fixture_dir = env["repo_root"] / env["fixture_dir"]
    (fixture_dir / "SecondEvent.json").write_text(
        json.dumps({"different": "shape"}), encoding="utf-8"
    )
    entry = verify.load_matrix(env["matrix_path"])["claude_code"]
...
def test_verified_result_for_one_client_cannot_promote_another(verify_env):
    ...
    codex_entry = verify.load_matrix(config.matrix_path)["codex"]
    _write_fixture(env["repo_root"], codex_entry.fixture_dir, SHAPE)
    codex_result = verify.verify_client("codex", codex_entry, config)
    assert codex_result.status == verify.STATUS_VERIFIED
    verify.promote(codex_result, codex_entry, config, now="2026-09-10T00:00:01Z")
