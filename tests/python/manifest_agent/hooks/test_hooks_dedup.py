...
def test_burst_replays_a_block_verdict_not_a_bare_allow(hook_harness, tmp_path):
    ...
    failing_config = hook_harness.root / "config/failing-project-checks.json"
...
def test_stop_continuation_runs_once_then_allows_the_active_continuation(
    hook_harness, tmp_path
):
    script = tmp_path / "always_fail_stop.py"
    script.write_text("raise SystemExit(1)\n", encoding="utf-8")
    config = hook_harness.root / "config/failing-stop-project-checks.json"
    write_custom_check_project(config, script)
    payload = _stop_payload(hook_harness.root)
    first = hook_harness.invoke(
        "claude-code",
        "Stop",
        payload,
        MANIFEST_HOOK_PROJECT_CONFIG=str(config),
    )
    duplicate = hook_harness.invoke(
        "claude-code",
        "Stop",
        payload,
        MANIFEST_HOOK_PROJECT_CONFIG=str(config),
    )
    continuation = hook_harness.invoke(
        "claude-code",
        "Stop",
    ...
    assert json.loads(duplicate.stdout)["decision"] == "block"
    assert json.loads(continuation.stdout) == {}
    assert len(hook_harness.receipts()) == 1


def test_expired_cached_verdict_is_recomputed(tmp_path):
    from manifest_agent.hooks.state import DedupKey, run_deduplicated

    state_dir = tmp_path / "state"
    key = DedupKey("claude_code", "PostToolUse", "digest", False, False)
    calls = 0

    def compute():
        nonlocal calls
        calls += 1
        return {"status": "PASS", "action": "allow", "reason": str(calls)}

    assert run_deduplicated(state_dir, key, compute).outcome["reason"] == "1"
    state_path = state_dir / "state.json"
    persisted = json.loads(state_path.read_text(encoding="utf-8"))
    next(iter(persisted["dedup"].values()))["ts"] = 0
    state_path.write_text(json.dumps(persisted), encoding="utf-8")

    assert run_deduplicated(state_dir, key, compute).outcome["reason"] == "2"
    assert calls == 2
