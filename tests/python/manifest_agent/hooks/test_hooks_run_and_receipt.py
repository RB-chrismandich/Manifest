...
def test_recursion_guard_refuses_a_real_nested_invocation(hook_harness, tmp_path):
    ...
    check_config = hook_harness.root / "config/recursion-project-checks.json"
...
def test_task_completed_uses_claude_codes_blocking_exit_contract(
    hook_harness, tmp_path
):
    script = tmp_path / "fail.py"
    script.write_text("raise SystemExit(1)\n", encoding="utf-8")
    config = hook_harness.root / "config/failing-task-project-checks.json"
...
def test_project_cannot_shadow_the_trusted_manifest_runtime(hook_harness):
    sentinel = hook_harness.root / "shadow-imported"
    (hook_harness.root / "manifest_agent.py").write_text(
