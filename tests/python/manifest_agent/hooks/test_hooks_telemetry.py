...
def test_telemetry_redacts_free_text_before_persisting(tmp_path, monkeypatch):
    from manifest_agent.hooks.telemetry import record_hook_telemetry

    state_home = tmp_path / "state"
    secret = "sk-" + ("a" * 40)
    monkeypatch.setenv("XDG_STATE_HOME", str(state_home))

    record_hook_telemetry(secret, "quick", "PASS", 0.01)

    persisted = (state_home / "manifest/telemetry/runs.jsonl").read_text()
