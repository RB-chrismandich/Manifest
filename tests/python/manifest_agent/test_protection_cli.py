...
def _write_workflow(
    ...
    *,
    ...
    aggregate_step_continue_on_error: bool = False,
    producer_continue_on_error: bool = False,
) -> None:
    agg = str(aggregate_continue_on_error).lower()
    agg_step = str(aggregate_step_continue_on_error).lower()
...
jobs:
  ...
  checks-aggregate-full:
    ...
    steps:
      - name: Aggregate shared checks
        continue-on-error: {agg_step}
        run: exit 1
""",
        encoding="utf-8",
    )
...
def _raw_from_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "required_status_checks": {
            ...
            "contexts": [item["context"] for item in checks["checks"]],
...
def test_schema_invalid_config_is_blocked_before_gh(env: dict[str, Any]) -> None:
    """A schema-invalid policy never reaches the GitHub API."""
    config = _config_dict()
    config["enforce_admins"] = None
    env["config_path"].write_text(json.dumps(config), encoding="utf-8")
    _write_workflow(env["workflow_path"])
