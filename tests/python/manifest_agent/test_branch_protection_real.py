...
def test_real_workflow_is_safe_to_require() -> None:
    """The aggregate and every producer must be blocking before activation."""
    config = protection.load_config(
        REPO_ROOT_MARKER / "config" / "branch-protection.json"
    )
    workflow_path = REPO_ROOT_MARKER / config["workflow"]
    ...
    assert protection.preconditions_met(preconditions)
    assert all(item.reason == "" for item in preconditions)
    aggregate = workflow_jobs[protection.aggregate_job_id(config)]
    assert "non-blocking" not in aggregate.name.lower()
...
def test_policy_codeowners_are_single_owner_without_placeholders() -> None:
    ...
    expected = {
        ...
        "/src/manifest_agent/protection.py",
        "/src/manifest_agent/protection_cli.py",
