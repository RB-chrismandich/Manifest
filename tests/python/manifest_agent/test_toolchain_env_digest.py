...
def test_current_python_provider_never_probes_an_executable(monkeypatch) -> None:
    """Provider identity comes from this interpreter, not a subprocess probe."""
    monkeypatch.setattr(
        ...
        type(
            ...
            (),
            {
                "run": lambda *_args, **_kwargs: (_ for _ in ()).throw(
                    AssertionError("probe ran")
                )
            },
        ),
        raising=False,
    )
    provider = toolchain_env_digest.external_python_provider(Path(sys.executable))

    assert provider.matches(provider.lock_record())


def test_external_python_provider_reports_exact_lock_identity() -> None:
    """A provider record contains implementation, build, and executable bytes."""
    provider = toolchain_env_digest.external_python_provider(Path(sys.executable))

    record = provider.lock_record()

    assert record["implementation"] == sys.implementation.name
    assert record["version"] == ".".join(map(str, sys.version_info[:3]))
    assert record["exe_sha256"]
    assert provider.matches(record)


def test_external_python_provider_rejects_missing_or_changed_pin() -> None:
    """A missing pin or a changed provider identity never matches."""
    provider = toolchain_env_digest.external_python_provider(Path(sys.executable))

    assert not provider.matches(None)
    changed = provider.lock_record() | {"exe_sha256": "0" * 64}
    assert not provider.matches(changed)
