...
def test_provision_uses_lexical_checkout_root_for_absolute_lock_from_foreign_cwd(
    ...
    result = _invoke(lock_path.absolute(), tmp_path / "store", "--offline", "--json")

    assert result.exit_code == 3, result.output
    assert json.loads(result.output)["status"] == "incomplete"


def test_provision_blocks_final_lock_symlink_with_structured_report(
    tmp_path: Path,
) -> None:
    archive_path = tmp_path / "demo.tar.gz"
    archive_path.write_bytes(_tar_gz_with("demo", b"content"))
    ...
    result = _invoke(lock_path, tmp_path / "store", "--offline", "--json")

    assert result.exit_code == 3
    report = json.loads(result.output)
    assert report["status"] == "blocked"
    assert "toolchain lock unavailable" in report["problems"][0]
