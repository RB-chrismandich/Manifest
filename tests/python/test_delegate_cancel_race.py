...
def test_cancel_waits_for_pgid_published_after_cancel_begins(env_factory):
    """The worker is SIGKILLed after Popen forked the backend (which holds the
    inherited backend.lock) but BEFORE the child wrote backend.pgid in preexec.
    A single read at cancel time would miss the pgid and _clear_pgid_tracking
    would orphan a write-capable backend under a terminal `cancelled` record.
    cancel must use the held lock as a handshake — wait for the pgid to publish,
    then kill the group. Here the orphan holds the lock and publishes
    backend.pgid 0.4s late; the record starts with NO pgid and no backend.pgid
    file. Pre-fix, the orphan survived cancel."""
    env = env_factory()
