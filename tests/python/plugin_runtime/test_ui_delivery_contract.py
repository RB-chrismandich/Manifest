...
def test_agent_output_schemas_are_strict_and_match_the_review_contract(
    ...
    review_schema = json.loads(
        review_schema_path(repo_root).read_text(encoding="utf-8")
    )
    reviewer_output = frontmatter(
        repo_root / "plugins/stitch-design/agents/ui-reviewer.md"
    )["output"]
    for review_document in (review_schema, reviewer_output):
        assert review_document["additionalProperties"] is False
        assert set(review_document["required"]) == set(review_document["properties"])
        assert_no_response_format_conditionals(review_document)
...
def test_omp_agents_are_constrained_to_their_declared_roles(repo_root: Path) -> None:
    builder = frontmatter(repo_root / "plugins/stitch-design/agents/ui-builder.md")
    reviewer = frontmatter(repo_root / "plugins/stitch-design/agents/ui-reviewer.md")
