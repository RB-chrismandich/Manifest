...
TASK_STATES = {
    "draft", "approved", "building", "candidate_ready", "reviewing", "repairing",
...
def test_task_schema_accepts_authorized_lifecycle_states(repo_root: Path) -> None:
    ...
    for state in TASK_STATES:
        ...
        if state in {"blocked", "failed"}:
            outcome = state
        assert_valid(validator, {**task, "state": state, "outcome": outcome})
...
def test_task_schema_requires_candidate_identity_and_allows_review_route(
    ...
    for state in ("candidate_ready", "reviewing"):
        assert_valid(validator, {**candidate, "state": state, "model_route": "@ui_review"})
    assert_invalid(validator, {**candidate, "model_route": "@unknown_route"})
    assert_invalid(validator, {key: value for key, value in candidate.items() if key != "candidate_revision"})
    assert_invalid(validator, {key: value for key, value in candidate.items() if key != "candidate_hash"})
    assert_invalid(validator, {**candidate, "candidate_hash": "sha256:8d5f2e"})


def test_task_schema_enforces_grant_and_required_contract_fields(repo_root: Path) -> None:
    validator = _validator(repo_root)
    task = _accepted_task()
    stitch_grant = {
        "project_id": "stitch-project-17",
        "expires_at": "2030-01-01T00:00:00Z",
        "mutations": [{"tool_name": "stitch.edit_screen", "input_hash": "sha256:8d5f2e", "max_uses": 1}],
    ...
    assert_invalid(validator, {**task, "stitch_grant": {**stitch_grant, "expires_at": "not-a-date"}})
    assert_valid(validator, {**task, "stitch_grant": stitch_grant})
    for field in ("allowed_paths", "forbidden_policy_paths", "capture_recipes", "evidence_refs"):
        assert_invalid(validator, {key: value for key, value in task.items() if key != field})
    assert_invalid(validator, {**task, "approved_check_recipes": ["npm run test:ui -- CheckoutCard"]})
...
def test_task_schema_requires_complete_hardened_check_recipes(repo_root: Path) -> None:
    ...
    assert_invalid(validator, _with_check_recipe(task, sandbox_image="registry.example/ui-check:latest"))
    assert_invalid(validator, _with_check_recipe(task, backend="docker", sandbox_image="registry.example/ui-check@sha512:0123456789abcdef"))
    assert_valid(validator, _with_check_recipe(task, backend="docker", sandbox_image="registry.example/ui-check@sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"))
    incomplete = {key: value for key, value in recipe.items() if key != "sandbox_image"}
    assert_invalid(validator, {**task, "approved_check_recipes": [incomplete | {"backend": "docker"}]})


def test_task_schema_requires_evidence_for_terminal_candidate_states(repo_root: Path) -> None:
    validator = _validator(repo_root)
    active = {
        "task_id": "ui-delivery-18", "state": "draft", "design_revision": "stitch-revision-72",
        "allowed_paths": ["src/components/CheckoutCard.tsx"],
        "forbidden_policy_paths": [".claude/settings.json"],
        "approved_check_recipes": [docker_check_recipe()], "capture_recipes": [capture_recipe()],
    ...
    candidate = {**active, "state": "candidate_ready", "candidate_revision": "git:4d2ce0b", "candidate_hash": "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}
    assert_valid(validator, candidate)
    for state in ("reviewing", "repairing"):
        assert_valid(validator, {**candidate, "state": state})
    ...
    for state, outcome in (("accepted", "verified"), ("failed", "failed"), ("blocked", "blocked")):
        ...
        assert_valid(validator, {**terminal, "evidence_refs": ["artifact://ui-delivery-18/result.json"]})


def test_task_schema_allows_pre_candidate_failures_with_evidence(repo_root: Path) -> None:
    validator = _validator(repo_root)
    terminal = {
        "task_id": "ui-delivery-19", "design_revision": "stitch-revision-73",
        "allowed_paths": ["src/components/CheckoutCard.tsx"],
        "forbidden_policy_paths": [".claude/settings.json"],
        "approved_check_recipes": [docker_check_recipe()], "capture_recipes": [capture_recipe()],
        "model_route": "@ui_code", "repair_cycles": 0,
