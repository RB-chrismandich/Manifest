...
def test_global_disable_suppresses_everything():
    ...
    prefs["enabled"] = False
    assert (
        gh.apply_gating([HINT_RULE, REMINDER_RULE], prefs, "pre-commit", {}, NOW) == []
    )


def test_disabling_reminders_keeps_hints():
    prefs = all_on()
    prefs["categories"]["reminders"] = False
...
def test_reminder_after_window_allowed():
    ...
    assert (
        len(gh.apply_gating([REMINDER_RULE], all_on(), "high-context", last, NOW)) == 1
    )
