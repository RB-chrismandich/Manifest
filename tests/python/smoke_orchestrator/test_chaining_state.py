...
def _cli(name, *args, **extra):
    return {
        "name": name,
        "type": "cli",
        "command": [sys.executable, CLI_TOOL, *args],
        **extra,
    }
        ...
        [
            {
                ...
                "steps": [
                    _cli(
                        ...
                        "emit",
                        "42",
                        captures={"invoice_id": r"invoice_id=(\d+)"},
                    ),
        ...
        [
            {
                ...
                "steps": [
                    _cli(
                        "a", "fail", "1", captures={"invoice_id": r"invoice_id=(\d+)"}
                    ),  # fails → no capture
                    _cli(
                        "b",
                        "emit",
                        "7",
                        needs=["invoice_id"],
                        captures={"token": r"invoice_id=(\d+)"},
                    ),
                    _cli(
                        "c", "ok", needs=["token"]
                    ),  # cascade-blocked: b never produced token
                ],
            }
        ],
    )
...
def test_persisted_state_reused_across_runs(tmp_path, monkeypatch):
    ...
    r1 = SmokeTestExecutor(catalog_dir=str(cat_dir), persist_state=True).run(
        "demo", tier="Lite"
    )
    ...
    r2 = SmokeTestExecutor(catalog_dir=str(cat_dir), persist_state=True).run(
        "demo", tier="Lite"
    )
