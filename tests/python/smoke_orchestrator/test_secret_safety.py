...
from smoke_orchestrator import report as report_mod  # noqa: E402
from smoke_orchestrator.executor import SmokeTestExecutor  # noqa: E402
from smoke_orchestrator.redact import Redactor  # noqa: E402
...
def test_secret_absent_from_all_output_sinks(tmp_path, monkeypatch):
    ...
    _write(tmp_path, "demo", [{
        "id": "login", "tier": "Lite", "steps": [
            {"name": "auth", "type": "cli", "sensitive": True,
             "command": [sys.executable, "-c", leak, "${env.BILLING_TOKEN}"]},
        ],
    }])
    redactor = Redactor()
    rep = SmokeTestExecutor(catalog_dir=str(tmp_path)).run("demo", tier="Lite", redactor=redactor)
...
def test_sensitive_capture_never_persisted(tmp_path, monkeypatch):
    ...
    _write(cat_dir, "demo", [{
        "id": "login", "tier": "Lite", "steps": [
            {"name": "auth", "type": "cli", "sensitive": True,
             "command": [sys.executable, "-c", "print('token=topsecret-XYZ')"],
             "captures": {"token": r"token=(\S+)"}},
        ],
    }])
    rep = SmokeTestExecutor(catalog_dir=str(cat_dir), persist_state=True).run("demo", tier="Lite")
    assert rep.exit_code == 0

    state_file = tmp_path / "state" / "smoke" / "state" / "demo.json"
    if state_file.exists():  # sensitive value must never be written, even with persist on
...
def test_sensitive_ref_without_env_source_fails_clearly(tmp_path):
    ...
    _write(tmp_path, "demo", [{
        "id": "login", "tier": "Lite", "steps": [
            {"name": "auth", "type": "cli", "sensitive": True,
             "command": [sys.executable, CLI_TOOL, "echo", "${env.DEFINITELY_MISSING_SECRET}"]},
        ],
    }])
