...
def test_parse_transcript_skips_plaintext_noise_line(tmp_path):
    # A plain-text noise line is skipped because json.loads raises JSONDecodeError.
    # Raw-write so the leading char is non-JSON.
...
def test_parse_transcript_keeps_whitespace_padded_json(tmp_path):
    # A valid JSON line with leading whitespace must STILL be parsed by json.loads.
    # Raw-write to preserve the leading whitespace.
