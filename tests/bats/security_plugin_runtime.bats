...
@test "security instructions use local doctrine and qualified skill interfaces" {
    run bash -c "grep -R -nE '(configs/claude|~/.claude/(scripts|references)|manifest '"parallel-agent"'|parallel_'"agent.py"'|learning_capture.sh|plugins/manifest-ops)' '$BUNDLE/skills' --include='SKILL.md' || true"
    assert_output ""
