...
@test "on-demand review invokes one reviewer for two artifacts" {
    mkdir -p "$SANDBOX/specs/001"
    printf 's\n' > "$SANDBOX/specs/001/spec.md"
    printf 'p\n' > "$SANDBOX/specs/001/plan.md"
    cat > "$SANDBOX/agy" <<'STUB'
#!/usr/bin/env bash
printf 'review\n' >> "$SPEC_REVIEW_CALL_LOG"
cat >/dev/null
printf 'NO_ISSUES\n'
STUB
    chmod +x "$SANDBOX/agy"
    cat > "$SANDBOX/manifest" <<'STUB'
#!/usr/bin/env bash
printf '%s\n' '{"agents":{"legacy":{"status":"complete","output":"NO_ISSUES"}}}'
STUB
    chmod +x "$SANDBOX/manifest"
    PATH="$SANDBOX:$PATH" \
    SPEC_REVIEW_CALL_LOG="$SANDBOX/calls" \
    ...
    assert [ "$(wc -l < "$SANDBOX/calls")" -eq 1 ]
}
