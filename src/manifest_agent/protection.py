...
DEFAULT_SCHEMA_PATH = REPO_ROOT / "schemas" / "branch-protection.schema.json"
...
def load_config(config_path: Path) -> dict[str, Any]:
    """Parse and validate the declared branch-protection target JSON."""
    config = json.loads(config_path.read_text(encoding="utf-8"))
    schema = json.loads(DEFAULT_SCHEMA_PATH.read_text(encoding="utf-8"))
    Draft202012Validator.check_schema(schema)
    errors = sorted(Draft202012Validator(schema).iter_errors(config), key=str)
    if errors:
        raise ValueError(f"config schema validation failed: {errors[0].message}")
    return config
...
def parse_workflow(workflow_path: Path) -> dict[str, JobInfo]:
    ...
    for job_id, spec in jobs.items():
        ...
        step_continue_on_error = any(
            bool(step.get("continue-on-error", False))
            for step in steps
...
def resolve_contexts(
    ...
    """Resolve job ids to unique status-check names or fail closed."""
    ...
    contexts = tuple(workflow_jobs[job_id].name for job_id in required_jobs)
    duplicates = sorted(
        context for context in set(contexts) if contexts.count(context) > 1
    )
    if duplicates:
        raise BlockedError(f"duplicate required check name(s): {', '.join(duplicates)}")
    return contexts
...
def build_payload(config: dict[str, Any], contexts: Sequence[str]) -> dict[str, Any]:
    """Build the exact REST body, binding checks to their trusted GitHub App."""
...
def get_live(gh_runner: GhRunner, repo: str, branch: str) -> LiveResult:
    ...
    combined = f"{stdout}\n{stderr}".lower()
    if "http 404" in combined and "not protected" in combined:
...
def diff_settings(
    ...
    for key, proposed_value in proposed.items():
        ...
        elif path == "required_status_checks.checks" and sorted(
            live_value or [], key=lambda item: (item["context"], item["app_id"])
        ) == sorted(proposed_value, key=lambda item: (item["context"], item["app_id"])):
            continue
...
def apply_protection(
    ...
    see `test_protection_cli.py::test_put_literal_confined_to_apply_protection`,
    which inspects the module source and asserts the HTTP method literal below
