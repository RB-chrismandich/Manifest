...
def _checked_relative_path(store: Path, relative_path: str) -> Path | None:
    ...
    Store manifests are untrusted. A lexical relative path is insufficient:
    an intermediate or terminal symlink can still escape the store.
    """
    ...
    try:
        candidate = (store / relative_path).resolve(strict=True)
    except OSError:
        return None
