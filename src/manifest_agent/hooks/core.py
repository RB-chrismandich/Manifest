...
def adapter_deadline() -> float:
    return time.monotonic() + default_timeout_seconds()


def remaining_seconds(deadline_monotonic: float) -> float:
    remaining = deadline_monotonic - time.monotonic()
    if remaining <= 0:
        raise ProtocolError("adapter deadline exceeded")
    return remaining


def read_bounded_stdin(stream, deadline_monotonic: float | None = None) -> bytes:
    """Read at most STDIN_CAP+1 bytes without waiting past the adapter deadline."""
    try:
        fd = stream.fileno()
        ...
        while remaining:
            ...
            if not readable:
                raise ProtocolError("adapter deadline exceeded while reading stdin")
...
def _compute_verdict(request: EventRequest, root: Path, digest: str) -> dict:
    ...
    status, diagnostics = run_manifest_check(
        ...
        base="HEAD",
        cwd=root,
        timeout_seconds=remaining_seconds(request.deadline_monotonic),
    )
    ...
    write_receipt(request.state_dir, receipt, request.deadline_monotonic)
...
def process_event(request: EventRequest) -> AdapterOutcome:
    ...
    dedup_key = DedupKey(
        ...
        is_continuation=bool(request.payload.get("stop_hook_active", False)),
    )
    verdict = run_deduplicated(
        request.state_dir,
        dedup_key,
        lambda: _compute_verdict(request, root, digest),
        request.deadline_monotonic,
    )
