...
def _repo_path(repo_root: Path, path: str | Path, *, strict: bool) -> Path:
    root = repo_root.resolve(strict=True)
    candidate = (root / path).resolve(strict=strict)
    try:
...
def _fixture_files(entry: ClientEntry, repo_root: Path) -> tuple[Path, ...]:
    try:
        directory = _repo_path(repo_root, entry.fixture_dir, strict=True)
    ...
    for path in sorted(directory.glob("*.json")):
        try:
            resolved = _repo_path(repo_root, path, strict=True)
        except (OSError, ValueError):
            return ()
...
def _unavailable(
    client: str, executable: str | None, version: str | None, reason: str
) -> VerifyResult:
    return VerifyResult(STATUS_UNAVAILABLE, client, executable, version, reason=reason)
...
def _primary_fixture(entry: ClientEntry, repo_root: Path) -> dict | None:
    files = _fixture_files(entry, repo_root)
    if len(files) != 1:
        return None
    try:
...
def _shape_result(
    client: str, executable: str, version: str, expected: dict, actual: dict
) -> VerifyResult:
...
def verify_client(
    ...
    if executable is None:
        return _unavailable(
            ...
            None,
            None,
            f"no executable found among {list(entry.executable_candidates)}",
        )
    ...
    if version is None:
        return _unavailable(
            ...
            executable,
            None,
            "version probe produced no match for version_pattern",
        )
    if not entry.protocol_probe_argv:
        return _unavailable(
            client,
            executable,
            version,
            "no protocol_probe_argv configured (owner input required)",
        )
    fixture_files = _fixture_files(entry, config.repo_root)
    if len(fixture_files) != 1:
        return _unavailable(
            client,
            executable,
            version,
            "protocol verification requires exactly one JSON fixture; "
    ...
    if expected is None:
        return _unavailable(
            ...
            executable,
            version,
            f"vendored fixture under {entry.fixture_dir} is invalid",
        )
    ...
    if actual is None:
        return _unavailable(
            ...
            executable,
            version,
            "protocol probe produced no parseable JSON object",
        )
    return _shape_result(client, executable, version, expected, actual)
...
def promote(
    ...
    *,
    ...
    if result.client != entry.key or entry.name != entry.key:
        raise ValueError("verified result and client entry do not match")
    if not _SAFE_COMPONENT.fullmatch(entry.name) or not _SAFE_COMPONENT.fullmatch(
        result.version
    ):
        raise ValueError("client name and version must be safe path components")
    timestamp = now or _utc_now_iso()
    source_files = _fixture_files(entry, config.repo_root)
    if len(source_files) != 1:
        raise ValueError("promotion requires exactly one validated JSON fixture")
    target_dir = _repo_path(
        config.repo_root,
        Path("tests") / "fixtures" / "hooks" / entry.name / result.version,
        strict=False,
    )
...
def is_promotion_recorded(client: str, version: str, config: VerifyConfig) -> bool:
    ...
    if entry.verified_version != version or not entry.verified_at:
        return False
    try:
        source_doc = _repo_path(
            config.repo_root, Path(entry.fixture_dir) / "SOURCE.md", strict=True
        )
    except (OSError, ValueError):
        return False
    if not source_doc.is_file() or source_doc.is_symlink():
        return False
    try:
