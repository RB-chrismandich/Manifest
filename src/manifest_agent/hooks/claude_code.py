...
def _validate(payload: dict, event: str) -> None:
    ...
    for name in _BOOL_FIELDS:
        if name in payload and not isinstance(payload[name], bool):
            raise core.ProtocolError(f"field {name!r} must be a boolean")
...
def main(argv: list[str]) -> int:
    if len(argv) != 1:
        ...
        return 0
    event = argv[0]
    deadline = core.adapter_deadline()
    try:
        payload = core.parse_event_object(
            core.read_bounded_stdin(sys.stdin.buffer, deadline)
        )
