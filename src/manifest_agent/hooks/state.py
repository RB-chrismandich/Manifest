...
def acquire_lock_until(fd: int, deadline_monotonic: float | None) -> None:
    ...
    while True:
        try:
            ...
            if remaining <= 0:
                raise TimeoutError(
                    "adapter deadline exceeded waiting for state lock"
                ) from None
...
def run_deduplicated(
    ...
    fd = _lock(state_dir)
    try:
        acquire_lock_until(fd, deadline_monotonic)
        state = _load(state_dir)
        _prune(state)
        cache_key = _dedup_key(key.client, key.event, key.digest)
        stop_key = f"{key.client}\0{key.event}"
        if key.is_stop and key.is_continuation:
            return Verdict(
                replayed=True,
                outcome={
                    "status": "SKIPPED_CONTINUATION",
                    "action": "allow",
                    "reason": "active stop continuation will not re-enter checks",
                },
            )
        ...
        outcome = compute()
        if key.is_stop and outcome.get("action") == "block":
