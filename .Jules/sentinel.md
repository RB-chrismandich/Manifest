...
## 2026-02-28 - Command Injection Fix in parallel_agent.sh
**Vulnerability:** Similar to the previous finding, `configs/claude/scripts/parallel_agent.sh` contained an unsafe `eval "$config_settings"` execution for parsing the YAML configuration `services.yml`.
**Learning:** Using `eval` on dynamically generated strings from external files is dangerous, even if generating assignments via awk.
**Prevention:** Refactored the parsing block to use a `while read` loop combined with `case` to directly and securely assign values to target shell variables, entirely avoiding the use of `eval`.
