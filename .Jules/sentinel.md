...
## 2026-03-15 - Insecure Temporary Directory Creation in Python (CWE-377)
**Vulnerability:** The script `configs/claude/scripts/parallel_agent.py` used a predictable PID-based naming convention (`Path(f"/tmp/.claude_agent_outputs_{os.getpid()}")`) for its fallback output directory.
**Learning:** Similar to shell scripts, Python scripts using predictable names in shared directories like `/tmp` are vulnerable to symlink attacks. An attacker can pre-create a symlink with the predicted name, leading to files being written with elevated permissions or sensitive data being leaked/corrupted.
**Prevention:** Always use `tempfile.mkdtemp()` or `tempfile.NamedTemporaryFile()` to create temporary directories/files. They guarantee a unique name and set safe permissions atomically, eliminating predictable path vulnerabilities.
