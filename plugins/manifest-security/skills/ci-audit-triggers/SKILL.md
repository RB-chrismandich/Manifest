...
For three or more workflow files, audit one workflow per review unit and merge
the findings. Below that threshold, audit inline. If structured output is
unavailable, perform the same review inline and report `DEGRADED`.

When dispatching, invoke `manifest-workspace:parallel-agent` with one workflow
per review unit and merge its structured findings.
