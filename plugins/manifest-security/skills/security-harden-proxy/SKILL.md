...
4. **Override the log formatter to log message-only** — ensure the access/error logger prints just the human message,
   not the request line or headers, and route it to stdout for container capture.
5. **Stream and cap untrusted payloads** — when the upstream returns a large/unbounded body, iterate it line-by-line and
