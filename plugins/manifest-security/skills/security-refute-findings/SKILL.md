...
the refutation rules above. Use `hub` only to coordinate or wait. If `task` is
unavailable, perform the same refutation inline and report `DEGRADED`. Below the
threshold, refute inline.
