...
**Purpose**: Package the security-relevant pre-write doctrine with the bundle
that consumes it, without requiring another plugin or a shared home config.
...
Apply these articles during pre-write reasoning and security review. When the
selected installation also exposes automated constitution checks, consume their
results; when it does not, apply the same doctrine inline and report the absent
automation as degraded rather than pretending a hook ran.
...
**Automated**: none here — pinning is enforced by the version-pin skill and the CI
lockfile gate.
