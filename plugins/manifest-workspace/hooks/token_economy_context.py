#!/usr/bin/env python3
"""Fail-open SessionStart delivery for the pinned token-economy guidance.

Bundle-local by design (spec 2026-09-07 modern-model-plugin-simplification,
Task 4): this is the plugin-only-install counterpart to the always-on
`## Token Economy (always on)` bullets already shipped in the bootstrap-
deployed CLAUDE.md/AGENTS.md. Claude Code and Codex both execute a plugin's
`hooks/hooks.json`-style SessionStart declaration; that is the verified
delivery surface this script fills, not a custom metadata field.
"""
...
def render_instructions(guidance_path: Path | None = None) -> str:
    path = guidance_path or _root() / "guidance" / "token-economy.md"
...
def record_hook_failure(state_root: Path, diagnostic: HookDiagnostic) -> None:
    ...
    path = state_root / "diagnostics" / "manifest-workspace-token-economy.json"
...
def main() -> int:
    ...
    try:
        ...
        sys.stdout.write(
            f"Manifest token-economy guidance v{VERSION}\n\n{render_instructions()}\n"
        )
        return 0
