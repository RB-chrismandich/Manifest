...
def _load_template(filename: str) -> str:
    """Load a wrapper template's source text from templates/<filename>.

    Templates use ``.format()``-style placeholders with ``{{``/``}}``
    escaping for literal braces in the generated bash/Python — callers keep
...
BASH_TEMPLATE = _load_template("cli_wrapper_bash.sh.tmpl")

# Python wrapper template (for systems without jq)
PYTHON_TEMPLATE = _load_template("cli_wrapper_python.py.tmpl")

# Minimal shell wrapper (no jq dependency)
MINIMAL_TEMPLATE = _load_template("cli_wrapper_minimal.sh.tmpl")
...
if __name__ == "__main__":
    ...
    if not original:
        print(
            f"Error: Cannot find original binary for '{args.cli_name}'", file=sys.stderr
        )
        sys.exit(1)
