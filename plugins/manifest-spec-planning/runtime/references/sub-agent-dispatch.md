...
> restating these rules. Packaged with the Spec Planning bundle.
...
| **`[[skill:manifest-workspace:parallel-agent]]`** | External multi-CLI cross-verification (Gemini/Cursor/Codex/Antigravity) with consensus scoring | Independent cross-model verification of one artifact/decision | Cross-platform |
...
|-----------|-----------|-------|
| Parallel information-gathering / research / broad audit (many independent items) | Native Task sub-agents | One sub-agent per item/batch. On Gemini/Codex/Agy → `[[skill:manifest-workspace:parallel-agent]]` or inline. |
...
| Independent cross-model verification of a security-sensitive, architectural, or >200-line change | `[[skill:manifest-workspace:parallel-agent]]` | Required by the constitution's Tier-1 gate (Principle II). Not native sub-agents. |
...
| Claude Code | Yes | — |
| Cursor | Yes | — |
| Gemini CLI, Codex, Antigravity | No | `cddl_invoke.py`, `[[skill:manifest-workspace:parallel-agent]]`, or inline |

Never leave an assistant without an executable path. Headless seams share
the owning bundle JSON config → `cli_agents` and `SYNTH_*` / `CDDL_INVOKE_*` / `EVOLVE_*` env overrides.
...
(this reference): native Task on Claude Code/Cursor, or
`[[skill:manifest-workspace:parallel-agent]]` / `cddl_invoke.py` / inline on other assistants. Sub-agents execute
...
python3 -m json.tool ../config/review_models.json
```
