...
cmd_acquire() {
    ...
    # attempt itself could not land (e.g. the backend only pre-provisions the
    # static `loop-active` label, never the dynamic `loop-active:<epoch>:<tok>`
    # name this call requests — labels.yml:58-61). That is a DEGRADED condition,
    # not evidence of contention, so it gets its own exit code (2) rather than
    # reusing the "held" code (1). A caller for whom this lock only prevents
    # duplicated work (not anything irreversible) can choose to proceed on 2.
