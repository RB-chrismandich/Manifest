...
#   --config <path>    Path to labels.json (default: bundle default)
#   --validate         Only validate — report missing labels, don't create

set -euo pipefail

err() { if [[ -t 2 ]]; then printf '\033[0;31m%s\033[0m\n' "label-sync: $*" >&2; else printf '%s\n' "label-sync: $*" >&2; fi; }
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h | --help)
            ...
            echo "  --config <path>    Path to labels.json"
...
find_labels_file() {
    ...
    echo "${FORGE_CONFIG_DIR}/labels.json"
}

# --- Parse labels.json with python3 ---
...
linear_available() {
    # The command owns authentication; this dispatcher never reads credentials.
