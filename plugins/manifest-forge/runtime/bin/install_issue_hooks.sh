...
# Coordinators pass an active-harness settings path explicitly. The default is
# an isolated XDG document, never an implicit write into a harness home.
SETTINGS_FILE="${ISSUE_HOOKS_SETTINGS:-${XDG_CONFIG_HOME:-${HOME}/.config}/manifest/forge/hooks.json}"
...
Usage: install_issue_hooks.sh <--enable [--native] | --remove> [--settings PATH]
  ...
  --settings P  Target active-harness settings JSON (default: XDG Forge hooks.json)
USAGE
}
...
def _same_hook(existing, wanted):
    ...
    same hook registers under different absolute bundle paths. Comparing full strings
