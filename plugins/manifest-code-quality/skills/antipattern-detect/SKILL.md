...
```bash
[[skill:learning-capture]] query --category antipattern --language <LANG> --format llm
```

- If a matching entry exists (same category + language + similar description), increment it:

  ```bash
  [[skill:learning-capture]] increment <ID>
  ```
...
Regenerated by `[[skill:learning-capture]] sync-docs`. Do not edit this file directly.
