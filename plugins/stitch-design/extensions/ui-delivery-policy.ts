...
const taskPath = (cwd: string, taskFile: string) => join(cwd, taskFile);
...
export default function uiDeliveryPolicy(pi: ExtensionAPI): void {
  pi.registerTool({ name: 'ui_delivery_status', label: 'UI delivery status', description: 'Read bounded UI delivery task status.', parameters: pi.zod.object({}).strict(), approval: 'read', strict: true,
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const taskFile = (params as { taskFile?: string }).taskFile;
      if (!taskFile) return result(STATUS);
      const task = await loadTask({ repo: ctx.cwd, taskFile: taskPath(ctx.cwd, taskFile) });
      return result({ extension: STATUS.extension, taskId: task.task_id, state: task.state, outcome: task.outcome });
    } });
  pi.registerTool({ name: 'ui_apply_patch', label: 'Apply approved UI patch', description: 'Apply an authorized non-destructive unified diff.', parameters: pi.zod.object({}).strict(), approval: 'write', strict: true,
    async execute(_id, params, signal, _onUpdate, ctx) {
      const args = params as { taskFile: string; patch: string };
      const task = await loadTask({ repo: ctx.cwd, taskFile: taskPath(ctx.cwd, args.taskFile), mutation: true });
      for (const target of diffTargets(args.patch)) await authorizePath({ repo: ctx.cwd, task, path: target });
      await gitApply(ctx.cwd, args.patch, signal);
      return result({ taskId: task.task_id, operation: 'ui_apply_patch', outcome: 'mutation_unknown' });
    } });
  pi.registerTool({ name: 'ui_run_check', label: 'Run approved UI check', description: 'Run a named approved check in an OS sandbox.', parameters: pi.zod.object({}).strict(), approval: 'exec', strict: true,
    async execute(_id, params, _signal, _onUpdate, ctx) {
      const args = params as { taskFile: string; checkId: string };
      const task = await loadTask({ repo: ctx.cwd, taskFile: taskPath(ctx.cwd, args.taskFile) });
  ...
  pi.registerTool({ name: 'ui_capture', label: 'Capture approved UI evidence', description: 'Record only artifacts named by an approved capture recipe.', parameters: pi.zod.object({}).strict(), approval: 'exec', strict: true,
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      await appendEvidence({ repo: ctx.cwd, evidenceFile, record: { taskId: task.task_id, approvedDesignHash: task.design_revision, candidateRevision: task.candidate_revision ?? 'none', candidateHash: task.candidate_hash ?? 'none', modelRoute: task.model_route, operation: 'ui_capture', outcome: 'captured', elapsedMs: 0, artifacts, stdoutHash: 'sha256:', stderrHash: 'sha256:' } });
