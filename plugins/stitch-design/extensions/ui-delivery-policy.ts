...
async function freshResult(path: string, before: { mtimeMs: number; size: number } | undefined): Promise<void> { const stat = await lstat(path); if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1 || (stat.mode & 0o777) !== 0o600 || (before && before.mtimeMs === stat.mtimeMs && before.size === stat.size)) throw new Error('stale check result'); if (!exactResult(JSON.parse(await readFile(path, 'utf8')))) throw new Error('check result is unverified'); }
