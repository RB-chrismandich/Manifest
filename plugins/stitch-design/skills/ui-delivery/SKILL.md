...
2. **Approval:** create and approve a task manifest with `qualification_hash`, authorized paths, forbidden policy paths, approved fixed-argv checks, capture recipes, and `model_route: "@ui_code"`. If Stitch is used, bind each mutation to a one-use input-hashed grant and readback tools.
