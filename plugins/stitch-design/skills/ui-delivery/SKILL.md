...
1. **Preflight:** require `/stitch-design:ui-delivery`, `/stitch-design:ui-verification`, OMP-only `ui-builder` and `ui-reviewer`, custom tools `ui_apply_patch`, `ui_run_check`, and `ui_capture`, plus `references/task.schema.json` and `references/review.schema.json`. Before activating the extension, run `runtime/model-qualification/qualify-models.mjs` against the active OMP overlay and catalog, export its `qualificationHash` as `UI_DELIVERY_ACTIVE_QUALIFICATION_SHA256`, and copy that exact value to the required task-manifest `qualification_hash`. The extension rejects a missing or mismatched active hash for status, Stitch, patch, check, and capture; never source it from a model tool argument. If any asset is unavailable, blocks work; there is no unrestricted fallback.
2. **Approval:** create and approve a task manifest with the active `qualification_hash`, explicit non-root authorized paths (`allowed_paths: ["."]` is rejected), forbidden policy paths, approved fixed-argv checks, capture recipes, and `model_route: "@ui_code"`. If Stitch is used, bind each mutation to a one-use input-hashed grant and exact supported readback tools.
...
7. **Terminal evidence:** accept only verified reviewer evidence with `model_route: "@ui_review"`. Build checks bind to the equivalent `@ui_code` digest; captures bind to the current `@ui_review` digest. Failed checks yield `failed`; unavailable, skipped, timed-out, wrong-route, wrong-digest, or active-qualification checks are `unverified`, never pass; authorization or recovery uncertainty yields `blocked`.
