...
Receive the approved task, exact candidate revision/hash, and capture recipes from `/stitch-design:ui-delivery`. Use the OMP-only `ui-reviewer` read-only and validate its output against `references/review.schema.json`. Bind every capture and finding to that candidate identity; never write or request a retry without a new authorized repair cycle. The coordinator maps accepted to verified, repair_required to unverified, blocked to blocked, and failed to failed; skipped or unavailable evidence is unverified and prevents a green verdict. After two repair cycles with open findings, return blocked.
