...
async function canonicalRelative(root: string, candidate: string): Promise<string> {
  ...
  for (const part of parts) {
    ...
    if (resolvable) {
      try {
        ...
        canonicalParts.push(basename(current));
        continue;
      } catch (error) {
  ...
  return canonicalParts.join(sep);
}
...
export async function authorizePath({ repo, task, path }: { repo: string; task: { allowed_paths: string[]; forbidden_policy_paths: string[] }; path: string }): Promise<string> {
  ...
  const canonicalRel = await canonicalRelative(root, candidate);
  const protectedEntries = [...PROTECTED, ...task.forbidden_policy_paths];
  if (blocked(rel, protectedEntries) || blocked(canonicalRel, protectedEntries)) throw new Error('protected path');
