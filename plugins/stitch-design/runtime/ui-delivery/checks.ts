...
async function approvedExecutable(argv: string[]): Promise<{ executable: string; runtime: string }> {
  ...
  for (const candidate of candidates) {
    try {
      ...
      return { executable, runtime: homebrew ? '/opt/homebrew' : application ?? dirname(executable) };
    } catch {}
  }
