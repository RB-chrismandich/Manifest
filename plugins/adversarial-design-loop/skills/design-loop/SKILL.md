...
1. **Scaffold** — invoke the `loop-scaffold` skill: create the artifact
...
3. **Screen prompts** — invoke the `screen-prompts` skill: one colorless,
...
5. **Render-verify** — invoke the `render-verify` skill: the generator's own
...
6. **Review rounds** — invoke the `review-round` skill repeatedly: parallel
...
7. **Spec amendments** — whenever a round exposes a genuine hole in the
   upstream spec, invoke the `spec-amend` skill: record it, adopt the
...
8. **Deferred scope → production rounds** — scope may be deferred
   ...
   production round (`review-round` skill, production variant): mechanical
