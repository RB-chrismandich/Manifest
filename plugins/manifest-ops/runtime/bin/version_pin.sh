...
#   --config FILE        Override config (default: ../config/version_pin.json).
#
...
CONFIG="${VERSION_PIN_CONFIG:-${SCRIPT_DIR}/../config/version_pin.json}"
...
Usage: version_pin.sh [<path>...] [--check] [--requested NAME=VERSION]...
  ...
  --requested NAME=VER Pin NAME to an exact requested version (repeatable)
  --rule ID            Limit to one rule-set entry from the config
  --config FILE        Alternate version_pin.json
...
with open(sys.argv[1], encoding="utf-8") as config_file:
    cfg = json.load(config_file)
print(cfg.get("bypass_marker", "version-pin:ignore"))
PY
}
