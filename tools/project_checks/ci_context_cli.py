...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--project-config", type=Path, default=Path("config/project-checks.json")
    )
    arguments = parser.parse_args(argv)
    try:
        project_config = arguments.project_config.resolve(strict=True)
    ...
    except (CIContextBlockedError, OSError, ValueError) as error:
